...
| `flat-tail/flat-tail.png` | The FLAT list reaching the rest of the catalogue, at its end. **RE-BASED ON THIS BRANCH'S OWN STRUCTURE (the #505 fold).** #505 had this arm open a collapsed `Previous chats` and scroll, because on its list the scroll was the ONLY route and the section had to be opened first; this redesign replaced `Active chats` / `Previous chats` with the sections the view popover draws (`Running`, `Today`, `This week`, `Older`) and none of them collapses (`"Previous chats"` occurs 0 times at this branch's pre-fold head `6a4a176f6`). What withholds the tail here is the page LADDER, so the arm spends the ladder to the rows in hand and then presses the list's own foot control - the operator's "then user can click to load more" - and asserts what the DOM cannot say: the stand-in RECEIVED a request carrying the head's own cursor (`...&cursor=off%3A50`). Two things are REPORTED rather than asserted, each because it is not something this panel promises: the commit-time extension fires once by itself and takes the head to 100 held rows, after which the ladder's rung is below the rows in hand again and `ladderHoldsRowsRef` holds the scroll back; and an UNSCOPED extension is not durable under the head's own re-read, because on the unscoped list the tail's rows are head-owned and `headHeldRows` replaces them on the next `catalogue` frame (round 1, R1 - #505's accepted rule, unchanged by this fold). The region is put back at its end and ASSERTED there before the capture (round 4, D21: an absolute `scrollTop` is a position, not a promise). |
...
| `flat-tail-error` | `--tail-error --tail-delay-ms 400` | the flat list refusing to grow, at a speed a person could see |
