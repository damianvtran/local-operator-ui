...
| `flat-tail/flat-tail.png` | The FLAT list's own tail, which has no control by design (one scroller, one scope inside it): a scroll to the bottom of `Previous chats`, and rows that were not there before — `Chat 089` … with their `lopdev` captions. The run asserts the growth rather than photographing a scroll. Round 4 (D21) fixed WHAT it shows: the scroll was written once, before the page it asked for landed, so an absolute `scrollTop` left the viewport behind and the frame was of the middle of the list. The region is scrolled again after the growth and ASSERTED at its end before the capture, because "the frame shows the tail" is the whole point of the arm. |
