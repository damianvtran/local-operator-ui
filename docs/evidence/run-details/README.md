...
# Round-3 fixes: the acknowledgement wiring, the fold's `queued` word, the
# machine-word strip on the display path, and the tenth story
# (`restored-and-unrecognised`). This round bound 6041, asked for rather than
# inherited: 6031 and 6021 are held by other worktrees' Storybooks, and the port
# was read back off the banner rather than assumed. Ten stories over the two
# brand themes is twenty frames, and `restored-and-unrecognised`'s pair were
# added by this run (`addedFrames: 2` in `manifest.json`).
...
the eighteen frames the set then held came back BYTE-IDENTICAL, and the four that
differ are exactly the panel frames with a live running clock in them
(`both-in-flight` in
...
**That capture moved ALL TWENTY frames it held, and the reason is the ground.** #113 gave
...
differing pixels are the two whose CONTENT changed: `crowded` (**994 dark / 1,074
light**) and `subagents-only` (**4,048 / 4,232**), and `both-in-flight` (light,
**119**; dark, 1) whose running clock ticked. Every other frame is under 41 at
...
**Which capture each count was read off, because they are not interchangeable.**
The three light figures above are this head's (measured against `c1e648b49`, the
previous set, with the same `dwebp -ppm` decode); the round-4 capture's own
frames read `956 / 4,236 / 117` for the same three, and the earlier version of
this paragraph quoted those without saying so. The light side is the sensitive
one because a mid-tone ground carries a shadow's and a glyph's edge over more
pixels than a dark one does, so a re-encode shifts a few hundred of them while
the dark counts (994, 4,048) reproduce exactly in both sets. Treat any single
count here as ±a few pixels and the dark/light asymmetry as the signal.
...
hour, and it DELETES the set before re-taking it — so a sweep to add two frames
rewrites 516 nobody reviewed and buries the two that changed. `--only`
...
| [`header-trigger-failed`](header-trigger-failed/) | The unseen failure: an 8px `danger` dot on the button's top-right corner. It is not a count; it says a child failed and nobody has looked, and the trigger clears it when the panel opens — for the rows that open put on screen (§ 3.3). |
| [`restored-and-unrecognised`](restored-and-unrecognised/) | The three states a LIVE session cannot produce, in one roster, and the frame that completes `§6.4`'s nine marks: `paused` (the pause circle — **a shape no other state uses**, and the one of the three that also decides whether the trigger exists, since a restored pause is open work), `gone` (the dashed circle, the quiet mark for a row the graph swept without an outcome) and an unrecognised word (`reticulating`, the question mark; the tally prints the wire's own word rather than a name this model invented for it). All three take `ink-dim` or `ink-muted` — none is `danger`, and none is the `done` check: an outcome nobody recorded is not a success. This is also the frame for the restored row's SHAPE: every row carries context and cost but **no elapsed value**, because a durable-graph row has neither `start_time` nor `settled_at` (§ 8), and the panel omits a figure it does not have rather than zeroing it. The button is above these rows, raised by the `paused` child alone. |
...
| One grid | marks and state icons in one 16px column at x≈133..149; first-line text at x≈157-158 in **both** lists, and the plan's `+N more` with it | Measured: the roster's state icons ink at x134..148 with labels at 158 — including the three marks `restored-and-unrecognised` was added for (the pause circle at y89-102, the dashed circle at y121-134 and the question mark at y153-166, each within x134..147, on the same column as `crowded`'s six) — and the plan's marks now ink at x134..147 with item text at 157-158. Before the fix the plan sat 12px right of the roster (marks at `pl-6`, text at 170), which is what made one panel read as two lists. The plan's own indent lives on the second line — the blocked reason at x≈157 — not in the first column, and the phase's `+N more` follows the same rule: it inks at x157 in `todos-only` (both themes), where the phase HEADERS below it ink at x133, so the disclosure reads as the footer of the rows above it rather than as the next header. |
| Panel edge | the one shadow, and no focus ring | Measured in both brand themes by walking DOWNWARD from the panel's floor in `crowded` (a horizontal probe crosses the transcript the panel overlaps, which is what made the previous set's edge numbers a mixture of shadow and glyph ink): the ground runs from the shadow's darkest pixel at the edge back to the page's own ground — dark `(16,14,8)` just below the hairline → `(23,19,14)`, light `(217,214,206)` → `(245,239,230)`, with the falloff reaching the ground 21px below the floor in dark and 37px in light (the last row more than one unit per channel off `canvas`; at three units it is 21px / 29px), and the reach is uniform across the panel's **interior**: every interior column gives one of two numbers (19-23px in dark, 35-39px in light), while the two columns nearest the rounded frame read SHORTER (15px and 19px in dark) because the frame cuts the shadow there — so "uniform across the panel's span" was one word too wide for what the pixels support. Those two distances are one quantity only after the direction is named, and naming the token's parts is what makes them checkable: `--lo-overlay-shadow` is `0 12px 32px -12px`, so the nominal reach is `32 − 12 = 20`; the LATERAL reach the pixels show is 8px in dark and 24px in light, which is palette-dependent because the two shadows sit on grounds of different luminance (dark `rgb(0 0 0 / 0.6)` on a `(23,19,14)` ground is within a unit of it by 8px; light `rgb(20 17 12 / 0.25)` on a `(246,241,231)` ground still shifts it at 24px), and the DOWNWARD reach is the larger one because the token's 12px y-offset pushes the shadow down — 21 / 37 against 8 / 24. The measured 21 / 29 are the shoulder of that falloff rather than the blur itself, and `32 − 12 = 20` is arithmetic, not a reading. **The ground it returns to is `canvas`**, the working column's own ground since #113 — and it returned `canvas` on the previous set as well, to the byte: comparing the two sets at identical coordinates, the strip below the floor (y532–819) differs by ≤2/255 in 196 of the 161,280 dark pixels — encoder noise on the same gradient — while the light strip differs in **6,434** pixels with a max delta of 8, which is the same noise with more room to move on a mid-tone ground (an earlier version of this row read the light figure as "≤6/255 in 4,729" off the round-4 capture, and it does not reproduce on this head's frames) — and the measured reach is identical (21px / 37px at one unit in both sets). **What actually changed is the ground ABOVE the panel's floor.** In the previous set the story's column painted `surface` from y0 to y427 in every panel and header frame and the preview frame's `canvas` showed below y428, so the panel sat on `surface` for its upper 377px and on `canvas` for its floor (measured at x520, y200: `(29,25,20)` then, `(23,19,14)` now; at y428 it was already `canvas`); this head paints `canvas` under all of it (§ 5, and the story's own note), so the plane the panel is over is one plane instead of a seam. The previous row's `(24,21,13) → (30,26,21)` by x≈526 was that row's own HORIZONTAL probe — the reading this row's probe exists to reject as a measurement of the shadow. The panel's separation, measured as the modal colour of a panel-interior box against a ground box below the panel: dark L* 6.14 → 13.62 (Δ 7.48) and light 95.28 → 99.43 (Δ 4.16), against the palette tokens' own `elevated`-over-`canvas` step of 7.90 (dark) and 4.73 (light) and their `elevated`-over-`surface` step of 4.41 / 2.10 — which is the ≈4.4 / ≈2.1 the previous set's frames certified, on the wrong plane. Read that pair as AN APPROXIMATION and read the capture off it: it is the panel's modal colour against the `surface` plane around the panel's upper body, where the two palettes' bounds are wide (this head's frames read 4.07 dark / 1.85 light on a text-free band at `x0..112`, and 4.57 / 2.62 at the `x520, y200` point above; the round-4 capture read 4.50 / 2.07 there). The direction is what survives the approximation, and the tokens settle it exactly: `elevated` clears `canvas` further than `surface` in both palettes. No frame in the set — all twenty, panel or header — contains an accent pixel at all: the capture opens the panel programmatically, so the trigger is never `focus-visible`, and its ring is therefore neither shown nor claimed by these frames (it is measured in `docs/run-details.md` § 7 instead). What that does prove is that the 2px ring the round before this one removed from the panel has not come back. |
...
- **Reduced motion.** The running mark is `motion-safe:animate-spin` and the
  ...
  The design's claim is that the SHAPES carry the states, and the nine states in
  these frames are all distinguishable without colour — the three that only
  `restored-and-unrecognised` carries (the pause circle, the dashed circle and
  the question mark) differ in shape from each other and from the spinner and
  the three settled marks.
