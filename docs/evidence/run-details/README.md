...
channel difference, larger than the 7-unit step), so at **8%** the only frames
with any differing pixel at all are the two whose CONTENT changed — `crowded`
(870 dark / 838 light) and `subagents-only` (3801 / 3979), whose rows are now
selected by the children's own clocks, and `both-in-flight` (light, 83) whose
running clock ticked. Every other frame is zero at that threshold. Read the
pixel counts at zero fuzz for the ground, and at 8% for the content; neither
number alone says what moved.
...
| Panel edge | the one shadow, and no focus ring | Measured in both brand themes by walking DOWNWARD from the panel's floor in `crowded` (a horizontal probe crosses the transcript the panel overlaps, which is what made the previous set's edge numbers a mixture of shadow and glyph ink): the ground runs from the shadow's darkest pixel at the edge back to the page's own ground — dark `(16,14,8)` just below the hairline → `(23,19,14)` by ≈30px out; light `(217,214,206)` → `(245,239,230)` by ≈34px out — which is the token's 32px blur less its `-12px` spread. **The ground it returns to is `canvas`**, the working column's own ground since #113; the same probe on the previous set ran back to `surface` (`(30,26,21)` / `(250,248,242)`), which is the plane the story used to paint under this popover. The panel's separation from that ground, measured as the modal colour of a panel-interior box against a ground box below the panel: dark L* 6.14 → 13.62 (Δ 7.48) and light 95.28 → 99.43 (Δ 4.16), against the palette tokens' own `elevated`-over-`canvas` step of 7.90 and 4.41 and their `elevated`-over-`surface` step of 4.41 and 1.78 — which is the 4.50 / 2.07 the previous set's frames certified, on the wrong plane. No frame in the set — all eighteen, panel or header — contains an accent pixel at all: the capture opens the panel programmatically, so the trigger is never `focus-visible`, and its ring is therefore neither shown nor claimed by these frames (it is measured in `docs/run-details.md` § 7 instead). What that does prove is that the 2px ring the round before this one removed from the panel has not come back. |
