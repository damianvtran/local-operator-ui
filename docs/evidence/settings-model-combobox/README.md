...
| [after/settings-model-open.png](after/settings-model-open.png) | The model list open, scoped by the hosting row's own DRAFT: every row is `anthropic/…`, with `anthropic, no credential` as each sub-line. The hosting row above it shows `Anthropic (Claude Pro/Max)` — the NAME — while the value stored is the bare id, which is why the scoping can be to `anthropic/` at all. |
