...
| [`turn-timestamps-narrow`](turn-timestamps-narrow/) | The same stamps at 420px, which is where the PLACEMENT claim is judged: the stamp is against the **bubble's** right edge rather than the row content box's, and at this width the two edges are close enough that the wrong one reads as correct. |
