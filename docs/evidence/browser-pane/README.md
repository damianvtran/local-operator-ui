...
| `scope-empty/` | The scope-empty state: no tab of this conversation is open while three exist, so the copy says so and offers `Show all tabs` rather than "No tabs are open." (7.2). Captured at the SAME 640x460 as `one-tab/` rather than tight to its content, because design round 2 (D9) is a comparison between exactly these two: the reported rect is `640x316 at 0,118` in BOTH, where before the fix the strip collapsed to its own padding with no tabs in it and the page stepped 32px — so these two frames, one press apart, are the pair that shows the page no longer moves |
...
| `with-approval/` | A live request for THIS conversation and a live request for another one, in the same projection: the tray's single card — the band a user with one request already knows — and the Approvals badge reading **1** while the other conversation's request is live. NOT the `for this conversation` sentence: that lives in the tray's header row, which renders only when the pane's own scope holds more than one request (`browser-approvals-tray.tsx`, `rows.length > 1`) or while the dock it opens is open, and this fixture puts one request in the pane's scope. The sentence's own frame is `pane-dock-open/`, where the dock holds it on screen (review round 2, MINOR 2) |
...
### The scope switch's track, in pixels

Design round 1 (D3) put a ring on the switch's track and design round 2 found it had
never painted: the primitive's list is Radix's `Tabs.List`, whose `RovingFocusGroup`
root writes an inline `style: { outline: "none" }` on the element it renders, and an
inline declaration beats every class. The ring is drawn on a wrapper Radix does not
own now, and the numbers below are the control role's own pixels in the switch's box
(x 150-460, y 0-40), counted from these frames with a tolerance of 8 per channel
because the frames are lossy WebP:

| Frame | Control-role pixels in the switch's box | The ring's own edges |
|---|---|---|
| `this-conversation/localOperatorDark.webp` BEFORE | 18 | none: the top and bottom edges of the ring do not exist |
| `this-conversation/localOperatorDark.webp` AFTER | 220 | 118px on the top edge, 70 on the bottom |
| `this-conversation/localOperatorLight.webp` BEFORE | 35 | none |
| `this-conversation/localOperatorLight.webp` AFTER | 284 | 118px on the top edge, 118 on the bottom |

The calibration is in the same frames: the same role's full-width rule (the
boundary under the URL bar) paints 638 pixels across a 640-pixel frame, so a 1px
rule of this colour is about a pixel per column and the ring's 118 is the track's
own width rather than a smear from the text inside it.
