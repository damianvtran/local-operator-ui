#!/usr/bin/env python
"""
Hold a REAL pending ``approval`` gate open for the click proof.

    python serve-approval-gate.py --port 14391 --scratch /tmp/approval-gate-rig \
        --token-file /tmp/approval-gate-rig/token \
        --result-file /tmp/approval-gate-rig/owner-answer.json
...
This is the backend half of ``scripts/click-proof.mjs`` for the approval card
whose two options this branch adds, and it is modelled on the ask set's sibling
(``docs/evidence/ask-options-live/harness/serve-gate.py``) rather than relearned:
a rig nobody can re-derive is not evidence, and the ask set already records what
the shapes are for.
...
a real ``_approval_gate`` — the same closure ``_install_gates`` registers on the
session in production, parking the same ``PendingRequest(kind="approval",
title, detail)`` a tool call parks, armed with the arguments the loop's own tier
gate passes. The browser answers it through the shipping ``/answers`` route with
the strict boolean that route takes.

WHAT IS NOT: the provider stream. Nothing here starts a turn — the gate is armed
on a standalone task that only records its answer — so the stream function is
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            "ANOTHER FRONT END ANSWERS FIRST, with this verdict (`approve` or "
            "`deny`), through the same route — and only then is the app's own "
            "press forwarded, so the app's request is the LOSER the route refuses. "
            "This is the shape the ask set's exclusivity probe measures "
            "(`probe-answer-exclusivity.py` fires the same two requests "
            "concurrently); here it is sequenced so a run can photograph what the "
            "page does with a refusal the owner's own state push has already "
            "stripped of its card. Nothing is faked: the winner's 200 and the "
            "loser's 409 are the route's own answers, and `owner-answer.json` "
            "records the verdict the owner kept."
        ),
    )
    parser.add_argument(
        "--answer-log-file",
...
async def main() -> None:
    ...
    class RigControl:
        ...
        def owner_answer(self) -> object:
            """The verdict the OWNER took, from the harness's own return value.

            `/rig-state` carries it so a run's record can say which boolean won
            rather than leaving that to a sibling file a reader has to find. The
            file is written by the harness's `_approval_gate` task when the gate
        ...
        def other_front_end(self, verdict: str) -> dict[str, object]:
            ...
            rig's bearer — and posts the verdict the caller named, so the request
            that wins the gate is a real one carrying a real boolean. Run in the
            ...
            try:
                    ...
                    body=json.dumps(
                        {
                            ...
                            # THE BOOLEAN IS THE WHOLE BODY, as the shipping
                            # route requires: it rejects `question_index` on an
                            # approval, and there is no label a daemon waits to
                            # hear for one (`Answer`'s own validator refuses a
                            # body with neither `value` nor `approved`, or with
                            # both).
                ...
                return {"status": answered.status, "verdict": verdict}
            except OSError as error:
                return {"error": str(error), "verdict": verdict}
            finally:
                connection.close()
    ...
    session.set_conversation_name("Approval-gate click proof", user_set=True)
    ...
    await session.transcript.append_message(
        Message(
            role="user",
            content=[TextContent(text="Clean the build output before rebuilding.")],
        )
    )
    ...
    # The gate a REAL bash call parks, argument for argument: the loop's tier
    # gate passes the tool's own name as `tool_name` and the tool's
    # `describe_approval` output as the description
    # (`_describe_shell_approval` → "run: <command>"), which is what the
    # committed live frames must be read against — a rig that invented a prose
    # title would photograph a card the app cannot produce.
    asking = asyncio.create_task(
        handle._approval_gate("bash", "run: rm -rf ./dist")
    )

    async def record() -> None:
        approved = await asking
        args.result_file.write_text(
            json.dumps({"approved": approved}, indent=2) + "\n"
        )
        print(f"ANSWERED {json.dumps({'approved': approved})}", flush=True)
...
def _test_model():
    ...
    return ModelSpec(provider="test", model_id="approval-gate-rig", context_window=100_000)
