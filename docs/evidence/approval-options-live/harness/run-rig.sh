#!/usr/bin/env bash
# Stand the approval-options-live rig up, drive one press, tear it down.
#
#   bash docs/evidence/approval-options-live/harness/run-rig.sh \
#     <out-dir> <expect|-> <answer-delay-ms> <order|-> <resolution> \
#     [extra serve-approval-gate.py args...]
#
...
# answers for WHOEVER is listening there. A sibling worktree's rig holding 5302 (the
...
SCRATCH="${RIG_SCRATCH_DIR:-/tmp/approval-gate-rig}"
PORT="${RIG_PORT:-5302}"
...
nohup "$PY" docs/evidence/approval-options-live/harness/serve-approval-gate.py \
  --scratch "$SCRATCH" \
  --session-id "$SESSION" \
...
LOCAL_OPERATOR_DESKTOP_TOKEN="$(cat "$SCRATCH/token")" \
  nohup node_modules/.bin/vite --strictPort --config docs/evidence/approval-options-live/harness/approval-gate.vite.mjs \
  >"$LOG/vite.log" 2>&1 &
VITE_PID=$!
