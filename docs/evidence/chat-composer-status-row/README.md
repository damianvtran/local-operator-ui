...
| `activity-motion-1/`, `activity-motion-2/` | **The spin, photographed at a HELD phase.** The pair is the same story, theme and rig, half a turn apart: `activity-motion-1` holds the shipped `animate-spin` at `0ms` of its own 1s cycle and `activity-motion-2` at `500ms`, through the rig's `{ liveMotion, phaseMs }` tuple, which skips the pre-shutter `animation: none` override for these two frames only. **Measured between them: 7,917 differing pixels in dark (3.6% of the 1000x220 frame) and 1,850 in light (0.84%), every one of them inside the two chips' marks** — clusters at x651-664 and x808-822, y53-67 in dark, the same two 14x14 boxes in light (per-channel threshold 8). **And the hold holds: re-running the capture reproduced all four frames BYTE-IDENTICAL, across three consecutive runs** (`cmp` on each file), which is the property a sampled pair cannot have. Round 2's M1 is why that sentence needed measuring: the rule this option used selected `.animate-spin`, the token the mark does NOT carry (`motion-safe:animate-spin` is), so it matched nothing and the pair was sampled — and a `paused` RULE would not have held it either, because such a rule freezes at whatever moment it lands. The phase is now SET through the Web Animations API (`pause()` and then `currentTime = phaseMs`, on EVERY mark in the row), and the rig FAILS the capture if the hold does not apply, so this option cannot quietly go back to matching nothing. Read the limits with it: this shows the shipped class rendering at two points of the shipped keyframes, in a Storybook fixture rather than a live app. Round 1's D3 was right that the previous set had no motion evidence at all — the reduced-motion frame was byte-identical to the plain one, so it proved nothing about the spin, and the claim lived only in a test. |
