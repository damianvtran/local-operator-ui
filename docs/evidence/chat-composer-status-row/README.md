...
| `activity-motion-1/`, `activity-motion-2/` | **The spin, photographed.** The pair is the same story, theme and rig, a half turn apart: `activity-motion-1` holds the shipped `animate-spin` at `-0ms` and `activity-motion-2` at `-500ms` of its own 1s cycle, through the rig's `{ liveMotion, phaseMs }` tuple, which skips the pre-shutter `animation: none` override for these two frames only. Measured between them: **51 structural pixels (dark) / 23 (light), in exactly two 12px-wide clusters at x651-662 and x808-819, y56-67 — the two chips' marks** — with the rest of the difference (266/316 pixels) sub-pixel text antialiasing at ≤50 per channel. Read the limits with it: this shows the shipped class rendering at two points of the shipped keyframes, which is as much as a rig that owns its own clock can honestly show. Round 1's D3 was right that the previous set had no motion evidence at all: the reduced-motion frame was byte-identical to the plain one, so it proved nothing about the spin, and the claim lived only in a test. The rig HOLDS the phase rather than sampling it because a sampled phase is not re-derivable — measured, three consecutive live captures of one story produced two distinct rotations and one repeat, so a committed pair could regenerate identical and quietly falsify this table. |
| `activity-chips-reduced-motion/` | The same four states with `prefers-reduced-motion: reduce`. **Say plainly what this frame is and is not**, which is what round 1's D3 asked for: it is byte-identical to `activity-chips/` (md5 `f6553f04140aa3558a41157157356ce2` in dark, both files 23094 bytes), so it is NOT a comparison and proves NOTHING about the animation — the rig injects `animation: none !important` before every shutter, which is why the animation is off in both frames. What it is worth is the one claim a still of that state can carry: with the reduced-motion media query active the mark renders, lands on a VISIBLE end state (the page's reduced-motion block CAPS durations rather than cancelling anything, so an element must never end invisible) and its shape still distinguishes running from parked. The spin itself is pinned as a class string in `scripts/composer-tabs.test.mjs` and photographed (held at a phase) in `activity-motion-1/2` above. |
