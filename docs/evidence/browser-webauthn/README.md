...
**EVERY `chooser-*` FRAME HERE IS COMMITTED AS `<name>/<theme>.webp`** (2026-09-25). The
evidence gate derives a frame's expected ground from its FILENAME, so the name has to be
the palette the frame was painted under; these were committed under the state they
photograph instead, and were renamed in place - the bytes are untouched.
...
- `chooser-answering-storybook/localOperatorDark.webp` + `chooser-several-named-storybook/localOperatorDark.webp` show
...
- `chooser-nameless-dark-storybook/localOperatorDark.webp` + `chooser-nameless-light-storybook/localOperatorLight.webp`
...
| --- | --- | --- |
| `chooser-several-named/localOperatorDark.webp` | two named passkeys | the everyday multi-match: the lead names the site, both rows carry a name and a login |
| `chooser-answered/localOperatorDark.webp` | after choosing | the dialog is gone, the credential reached Electron's callback, focus is back on the rail row it was on before |
| `chooser-nameless/localOperatorDark.webp` | three unnamed credentials | the lead says what the choice decides and carries the recovery clause in prose (design D11 / UX U9); every row's second line is the short "The site stored no name for this passkey." |
| `chooser-long-names/localOperatorDark.webp` | a long display name and a 76-character login | both rows wrap: `client=396 scroll=396`, `overflowing=false`, no text outside the panel |
| `chooser-many-accounts/localOperatorDark.webp` | twelve credentials | the list scrolls; the Touch ID sentence stays inside the panel (`notePinned=true`) |
| `chooser-queued/localOperatorDark.webp` | two requests at once | the second is COUNTED as waiting ("One more passkey request is waiting." — requests, not sites: N3/U7) instead of replacing the first |
| `chooser-oldest-expiring-while-queued/localOperatorDark.webp` | the older of two, seconds before it expires | both requests live, oldest first: `rows=2`, lead names `oldest.example.com` |
| `chooser-queued-oldest-expired/localOperatorDark.webp` | the older one has expired, the newer is still live | **the round-2 blocker measured**: `elapsed=61s open=true rows=2 expires=1` — the panel still shows `newer.example.com`, and no ending is on screen |
| `chooser-ending-after-the-queue/localOperatorDark.webp` | the newer one cancelled | the ending the live request was hiding becomes the panel: `rows=0`, "This passkey request expired", one Close |
| `chooser-ending-dismissed/localOperatorDark.webp` | the ending dismissed | the dialog is gone and the caret is back on the rail row — `activeElement=nav-item-chat` (UX U6) |
| `chooser-plain-expiry-ending/localOperatorDark.webp` | a single request, nobody answered, 60 s | the ending on its own — the shape a user actually meets, and the one round 2's check did not walk |
| `chooser-plain-expiry-dismissed/localOperatorDark.webp` | that ending dismissed | the dialog is gone and the caret is on the rail row: `activeElement=nav-item-chat`, with the instrumented tape showing the app's own `focus(nav-item-chat)` at the dismissal (UX round 3, U6) |
| `chooser-on-chat-route/localOperatorDark.webp` | raised while the browser surface is NOT mounted | `hash=#/chat`, `surface-mounted=false`, and the dialog is up and answerable |
| `chooser-after-return/localOperatorDark.webp` | back on the surface | byte-identical to `chooser-several-named/localOperatorDark.webp` — the same request, unchanged by the route change |
| `chooser-expired/localOperatorDark.webp` | 60.3 s later | the rows are replaced by the ending in words, not a live-looking dialog whose click would be discarded |
| `chooser-answering-storybook/localOperatorDark.webp` | the Answering state (stories file) | the rows' text takes the disabled ink and "Answering…" is pinned in the footer beside Cancel (design N1) |
| `chooser-several-named-storybook/localOperatorDark.webp` | the same dialog, enabled | the contrast half of that pair: the labels paint `ink` |
...
| UX U6 — dismissing an ENDING left the caret on `<body>` | `activeElement=nav-item-chat` after the ending's Close, and the frame `chooser-ending-dismissed/localOperatorDark.webp` |
