...
| [`wakes-many`](wakes-many/) | Nine schedules, which `MAX_WAKE_SCHEDULES = 16` makes an ordinary full-scheduler state rather than an extreme: six rows drawn (`WAKE_ROW_CAP`), then **`3 more wakes`** as a STATEMENT in the row-text column — the shared `Disclosure` primitive's disabled branch, the same treatment the plan's shed rows wear, and deliberately not the roster's `Show N more`, which IS a control: nothing in this pane can put a shed wake back. The tally still says `9 wakes armed` and counts the WHOLE list; the marker counts the difference. |
