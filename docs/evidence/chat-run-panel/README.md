...
(`local_operator/server/routes/desktop_sessions.py:349`) but the pairing, a real
turn with a live child and the CDP drive of the running app were not completed in
this window. The fixture-backed `reader-live` below is the rendering of that
state, not evidence of the pulse-to-transcript path; that path belongs to the
joint QA round, and the README will say which is which rather than blur them.
...
| [`todos-phased`](todos-phased/) | The plan with every phase named: three phase headers, each a **name alone** — no `· n/n resolved`. |
...
| [`reader-live`](reader-live/) | A running child's reader, **fixture page**: chrome bar with the back control and the breadcrumb, the facts row (state, label, elapsed, context, cost, `model_label`), the folded brief, then the child's transcript rendered through the parent's own path — durable rows, a `bash` tool row, the outcome text. |
| [`reader-settled`](reader-settled/) | The same child settled: the elapsed clock has stopped and the outcome block carries the final text. |
...
| [`mcp-unknown-status`](mcp-unknown-status/) | A word this build was not taught (`reticulating`): rendered verbatim in the quiet ink with the unknown mark, **no** remedy line, and the dot **on** in the closed frame's terms — the refusal is never to claim the good state, and never to paint a `danger` failure the renderer cannot name. |
...
| [`narrow-800`](narrow-800/) | The window floor, the rail expanded, the panel at its **320px minimum**. Measured: the hairline sits at x478 of the 800px frame, i.e. the pane occupies 322px including its 1px border. The name segments truncate (`Read invoice…`, `Compare against…`) while every number stays whole — `§ 8` makes the name the only segment allowed to shrink. |
...
| Claim | Measurement |
| --- | --- |
| The panel is 420px by default and takes its 320px floor at the window floor. | The vertical hairline's darkest column: **x856 of 1280** in `mcp-auth-required` (424px including the 1px `border-l` and antialiasing) and **x478 of 800** in `narrow-800` (322px on the same arithmetic). The 4px and 2px surpluses are the border plus the shadow the divider's handle paints over the seam. |
...
| The plan carries one count and no per-phase counts. | `todos-phased`, `todos-only` and the mixed-phase frame each show exactly one tally per section (`11 of 15 resolved · 1 dropped`) and three phase headers that are names alone. Pinned in the model test. |
