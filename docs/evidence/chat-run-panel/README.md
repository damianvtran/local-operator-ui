...
**The trigger's glyph was re-taken on the branch that swapped it, and the three `trigger-*` stories are the frames that carry the change.** `Activity` (a heartbeat, and therefore a liveness claim on a pane that holds a roster, a plan and MCP servers) became `Info` in `docs/composer-status-tabs.md` § 6, which is a mutation of the glyph inside its own 16x16 box and of nothing else. The re-take is `--only=chat-run-panel--trigger` in this set's own two brand themes — six frames — so the set is still the thirty-nine stories over two themes this file's capture section describes, where a twelve-palette run would have been a different set rather than a re-take of this one. "The icon is the only thing that may change in them" is a claim a reader should be able to check, so it is measured rather than asserted: four of the six differ only inside that box — `15x16+381+20`, two frames at `16x16+380+20`, and `15x16+781+20` — and the other two — `trigger-idle/localOperatorLight` and `trigger-open-hover/localOperatorDark` — differ over a wider box whose worst band measures a maximum of 17/255 and 16/255 at means of 0.15 and 0.21, which is the capture-to-capture anti-aliasing shape the round-2 paragraph above already measured rather than a second mutation. The grounds these rows make their claims about are untouched: the glyph is ink inside a box, and the box's ground is what the rows measure.
