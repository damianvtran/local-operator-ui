...
- **Diff body** (`_append_diff_body`, tool_card.py:2178-2225; the loop is :2208-2220): the nameless `---`/`+++` header pair is stripped **positionally** (only if lines 0 and 1 are exactly those). Then per line, the kind comes from the leading char `@` → `muted`, `+` → `success`, `-` → `danger`, else `dim` — and the WHOLE line is painted in that ink (`row.append(truncate_cells(line, line_width), style=ink)`, :2220); the function's own docstring says "only the leading marker character is coloured here" and is wrong about its own loop, so read the loop. `EXPAND_MAX_LINES = 40`, overflow `… N more diff line(s)`.

> NOTE: the line citations in this file predate the `tool_card.py` refactors and have drifted (this bullet pointed at :1988-2035, which is now inside the live block); only the diff-body bullet above was re-anchored, by the diff-body change, because it is the one that also stated the ink law backwards.
