...
Category taxonomy source: `_TOOL_CATEGORY` (tool_card.py:218-238), looked up case-insensitively via `_category_element` (tool_card.py:241-255). Any tool not listed → `tool.row.name_settled`. Liveness outranks identity: a running row never shows a category hue.
...
- **Diff body** (`_append_diff_body`, tool_card.py:3159-3207; the loop is :3189-3201): the nameless `---`/`+++` header pair is stripped **positionally** (only if lines 0 and 1 are exactly those). Then per line, the kind comes from the leading char `@` → `muted`, `+` → `success`, `-` → `danger`, else `dim` — and the WHOLE line is painted in that ink (`row.append(truncate_cells(line, line_width), style=ink)`, :3201); the function's own docstring says "only the leading marker character is coloured here" and is wrong about its own loop, so read the loop. `EXPAND_MAX_LINES = 40`, overflow `… N more diff line(s)`.

> NOTE: the line citations in this file predate the `tool_card.py` refactors and have drifted. TWO bullets have now been re-anchored, each re-measured against the file this branch ships: the diff-body bullet above (`_append_diff_body` :3159-3207, loop :3189-3201) and the taxonomy source in § 6 (`_TOOL_CATEGORY` :218-238, `_category_element` :241-255). Everything else still carries its pre-refactor numbers — re-measured on the same file, `truncate_cells` is :608 (this file says :390-417), `_search_result_output` is :804 (:586-613) and `_append_fetched_row` is :3095 (:631-697) — so read a line number in this file as a NAME and find the symbol, not as an address. The two that were re-anchored are the two a reader can be misled BY rather than merely inconvenienced by: the taxonomy pair is what the desktop port asserts against, tool name for tool name (`scripts/tool-row.test.mjs`), and the diff-body bullet is the one that also stated the ink law backwards.
