...
| `unknown-card/after-click/localOperatorDark.webp` | **The register on the card, which is the arm a plain press reaches.** The answer request is held past the app's own control deadline (`--hold-answers-ms 40000`) while the gate stays pending, so the card is still up and the outcome is unknowable — the exact state UX round 2's U7 and QA round 2's Q1 measured, where the app used to print `Your answer was not sent` and deny it in the next clause. The card now carries `Whether your answer landed is not knowable. The app waits up to 20 seconds for this request, and it was still running when the app stopped waiting. It may or may not have reached the server; check the result before repeating it.` The run asserts `report.cardUnknown: true` with `notSentCopy: []`, and its options stay disabled, so the control cannot repeat an answer whose fate is unknown (UX round 2, U9). **The wrap's own measurement** (design round 3, D14, whose round-2 D10 asked for it here): the two-line register occupies y **713-724** and y **733-745**, the card's bottom edge does not move (its last content row is **746** before and **745** after — the card is content-sized and grows upward), everything above it moves up **47 px** (the user's bubble ink 395-404 → 348-357, its timestamp 423 → 376), nothing clips, and the composer stays docked. The one-line case moves the same things **28 px**, which is the band's own reserved row, so the motion is the same motion on a plain press — and the pair that number compares is this set's own two single-line arms, `after-click/` (bubble ink y 697-709) against `refused-composer/after-click/` (y 669-681), so a reader can re-derive it from the frames rather than from this sentence (agent review round 4, R4-3). |
...
- `CLICK_PROOF_EXPECT` — what the surface must say, and the whole list, because a
subset of it is what round 3 found here: `silent` (a press the owner took is not
reported), `settled` (the pre-fix build's sentence, and the settled arm's own),
`moved-on` (the ask advanced past the question), `unknown` (the UNKNOWN register,
wherever it is carried), `not-sent` (the not-sent sentence in the composer's
band), `card-refusal` (that sentence written on the card that is still up),
`card-unknown` (the UNKNOWN register written on the card that is still up, which
is `unknown-card/`), and `-` (or unset) to record without asserting. The list is
the DRIVER's own — `scripts/click-proof.mjs` is the authority and `run-rig.sh`'s
usage line is its copy — and an unrecognised value no longer falls through to "no
contradiction": the driver checks it before anything runs and exits non-zero
(agent review round 3, C; QA round 3, Q3), which
`scripts/click-proof-expect.test.mjs` holds to the driver's own map and to that
usage line.
...
#    arms, `settled` on the build before the fix — and fail (writing
