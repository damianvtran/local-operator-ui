...
Twelve frames of the shipped renderer — served over Vite with `desktopProxyPlugin`
...
They are **six runs**: a pair for the ordinary ordering, a pair for the forced
losing ordering, and the four arms of the refusal itself — including the two no
...
| `refused-card/after-click/localOperatorDark.webp` | **A refusal that stays on the card.** The route refused with a bare `409` (`{"detail":"This answer belongs to an earlier session owner"}` — the epoch-rollover body, one of the three the route can send) while the gate is untouched, so the card is still up and holds `Your answer was not sent. This answer belongs to an earlier session owner.` The run asserts that reading: `report.cardRefusal: true` with `notSentCopy: []` — the sentence is on the page, the card is up, and it did **not** come from the composer's alert band. |
...
runs by construction (the same card), and their `after-click/` frames are the four
surfaces the sentence can land in. Only `refused-card` claims something other than
...
busy turn to produce it: the measured margin is small — this set's ordinary run
delivered the response 24.3 ms after the route answered it, with the card clearing
about 414 ms after the press (273 ms after that delivery) — and the
`lost-report-*` pair holds exactly 2638/2508 ms between the same two moments (the
forced delay), which inverts the order. A margin that size inverts under load.
The earlier round's 124 ms against 200 ms was the same race measured on a quieter
machine; `ask-answer.ts` now quotes this set's figures.
...
`old-settled` (the pre-fix wording, for the BEFORE run), `card-refusal` (the
refusal written on the card that is still up), `not-sent` (the card's own
sentence in the composer).
...
| the epoch was minted by a previous runtime instance | `{"detail":"This answer belongs to an earlier session owner"}` | the live epoch differs from the one the press sent | `Your answer was not sent. This answer belongs to an earlier session owner.` |
...
The rollover arm is the one the `refused-card/` frame photographs, because a
rolled-over runtime refuses with the gate still painted.
...
photographs the other one: the request is held past the renderer's own control
deadline (25 s — `desktopRequestTimeoutMs`), so no response ever comes back while
the backend is still alive and working. The app's own copy for that case already
...
`answerOutcomeIsUnknown` covers the three ways the app loses sight of a request
without the owner refusing it — no HTTP response at all, its own `deadline_exceeded`
(a `504` main synthesises) and its own `transport.failed` — and deliberately not
the statuses the BACKEND chose, including a daemon `503` and a coded `409`.
