#!/usr/bin/env bash
# usage: run.sh <scene> <pass> [window] [old-daemon]
cd "$(dirname "$0")"
R=$PWD
env -i HOME=$R/home PATH="$PATH" TERM=xterm-256color GIT_CONFIG_SYSTEM=/dev/null GIT_TERMINAL_PROMPT=0 TMPDIR=$R/tmp \
  ...
  QA_CAP_CASES="${QA_CAP_CASES:-}" QA_UI_TREE=/Users/damian/local-operator-ui-worktrees/qa482-r5-cdfce2 \
  QA_BACKEND_TREE=/Users/damian/local-operator-worktrees/qa482-r5-cdfce2 \
  bash -c 'mkdir -p "$HOME" "$TMPDIR"; bash ./capture-r5.sh "$@"' _ "$@" > "$2.out" 2>&1
echo "exit $?"
