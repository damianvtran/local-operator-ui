...
#: A PRIVATE PORT, not the shared 8080 (see the round-4 README): a peer session
#: held 8080 with a `lop serve` that does not take the shared lock, so the shared
#: port is not obtainable without touching another session's process. The built
#: app only reaches origins its CSP allowlists, so the CSP in `out/renderer/
#: index.html` (build output, gitignored) is widened for this port by the rig.
UI_PORT="${UI_PORT:-8177}"
DESK_STAMP="design482r4-$(uuidgen | tr A-Z a-z)"
...
env -i HOME="$RUN" LOCAL_OPERATOR_HOME="$RUN" LOCAL_OPERATOR_CONFIG_DIR="$CONFIG" \
  ...
  nohup "$PY" "$HERE/stub-d4.py" > "$RUN/logs/stub.log" 2>&1 &
STUB_PID=$!
...
env -u LOCAL_OPERATOR_CONFIG_DIR QA_PROXY_STATS="$STATS" QA_SEED_TURNS="${QA_SEED_TURNS:-0}" D4_CASES="${D4_CASES:-}" D4_THEME="${D4_THEME:-}" node scripts/renderer-driver-d4.mjs \
  --scene "$SCENE" \
