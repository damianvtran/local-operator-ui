...
def _script(body: dict) -> list[str]:
    ...
    # design-482-r3 markers, carried over: an exact row count (hard breaks, short
    # lines that never wrap) and the LONG answer paced so the stream can be sampled.
    import re as _re

    m = _re.search(r"NLINES(\d+)", last_user)
    if m:
        n = int(m.group(1))
        text = "  \n".join("Line %d of %d." % (i + 1, n) for i in range(n))
        return [
            _chunk({"role": "assistant", "content": text}),
            _chunk({}, finish="stop"),
            _chunk({}, usage={"prompt_tokens": 5, "completion_tokens": 5, "total_tokens": 10}),
        ]
    if "LONGSLOW" in last_user:
        out = []
        for i, piece in enumerate(LONG_PIECES):
            out.append(_chunk({"role": "assistant", "content": piece}) if i == 0 else _chunk({"content": piece}))
        out.append(_chunk({}, finish="stop"))
        out.append(_chunk({}, usage={"prompt_tokens": 21, "completion_tokens": 300, "total_tokens": 321}))
        return out

    # design-482-r4 additions (this scratch copy only): answers built so the panel's
    # ceiling CUTS INSIDE a specific block, which is what this round has to look at.
    # Each is returned in ONE piece so the answer settles quickly; the ceiling's cut
    # is a REST property, so streaming pace would add wait time and nothing else.
    def _grid(text):
        return [
            _chunk({"role": "assistant", "content": text}),
            _chunk({}, finish="stop"),
            _chunk({}, usage={"prompt_tokens": 15, "completion_tokens": 120, "total_tokens": 135}),
        ]

    _row = "Filler row {n}, one short sentence that fits a single line on purpose."
    m = re.search(r"LISTEDGE(\d+)", last_user)
    if m:
        n = int(m.group(1))
        fillers = "\n\n".join(_row.format(n=i + 1) for i in range(n))
        items = "\n".join("- list item %d of the block that reaches the edge" % (i + 1) for i in range(14))
        return _grid(fillers + "\n\n" + items + "\n\n" + _row.format(n=99))
    m = re.search(r"HEADEDGE(\d+)", last_user)
    if m:
        n = int(m.group(1))
        fillers = "\n\n".join(_row.format(n=i + 1) for i in range(n))
        body = "\n\n".join(_row.format(n=i + 50) for i in range(6))
        return _grid(fillers + "\n\n## A heading whose box the edge can land inside\n\n" + body + "\n\n## Second heading\n\n" + body)
    m = re.search(r"CODEEDGE(\d+)", last_user)
    if m:
        n = int(m.group(1))
        fillers = "\n\n".join(_row.format(n=i + 1) for i in range(n))
        code = "\n".join("const handler%d = (req, res) => run(req, res);" % i for i in range(12))
        return _grid(fillers + "\n\n```js\n" + code + "\n```\n\n" + _row.format(n=98))
    m = re.search(r"QUOTEEDGE(\d+)", last_user)
    if m:
        n = int(m.group(1))
        fillers = "\n\n".join(_row.format(n=i + 1) for i in range(n))
        body = "\n\n".join(_row.format(n=i + 60) for i in range(6))
        return _grid(fillers + "\n\n> a blockquote whose rule and box the edge can land inside\n\n" + body)
    m = re.search(r"SENTENCE(\d+)", last_user)
    if m:
        n = int(m.group(1))
        return _grid(" ".join("Sentence %d of one paragraph, of ordinary length." % (i + 1) for i in range(n)))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        for piece in _script(body):
            time.sleep(FIRST_DELAY if first else CHUNK_DELAY)
            first = False
