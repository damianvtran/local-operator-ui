#!/usr/bin/env bash
# Builds scripts/renderer-driver-d4.mjs in the design-round-4 worktree: a COPY of the
# driver at the PR head plus rounds 1-5's helper layer and scenes and round 4's own
# scene. The shipped driver and the app source are not modified.
set -euo pipefail
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TREE="$1"
python3 - "$TREE/scripts/renderer-driver.mjs" "$TREE/scripts/renderer-driver-d4.mjs" "$HERE" <<'PY'
import sys
...
for f in ("r4/helpers-r1.js", "r4/scene-r2.js", "r4/scene-r3.js", "r4/scene-r3-479.js", "r4/scene-r4.js", "r5/scene-r5.js", "scene-d4.js", "scene-r3design.js"):
    s += "\n" + open(f"{here}/{f}").read() + "\n"
open(dst, "w").write(s)
PY
node --check "$TREE/scripts/renderer-driver-d4.mjs" && echo "driver built and parses"
