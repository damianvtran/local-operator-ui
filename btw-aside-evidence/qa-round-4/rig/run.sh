...
env -i HOME=$R/home PATH="$PATH" TERM=xterm-256color GIT_CONFIG_SYSTEM=/dev/null GIT_TERMINAL_PROMPT=0 TMPDIR=$R/tmp \
  ...
  QA_UI_TREE=/Users/damian/local-operator-ui-worktrees/qa482-r4-a2df5f \
  QA_BACKEND_TREE=/Users/damian/local-operator-worktrees/qa482-r4-a2df5f \
  bash -c 'mkdir -p "$HOME" "$TMPDIR"; bash ./capture-r4.sh "$@"' _ "$@" > "$2.out" 2>&1
echo "exit $?"
