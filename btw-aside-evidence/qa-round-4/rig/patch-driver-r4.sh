#!/usr/bin/env bash
# Builds scripts/renderer-driver-qa4.mjs in the QA worktree: a COPY of the driver at
...
TREE="$1"
python3 - "$TREE/scripts/renderer-driver.mjs" "$TREE/scripts/renderer-driver-qa4.mjs" "$HERE" <<'PY'
import sys
...
open(dst, "w").write(s)
PY
node --check "$TREE/scripts/renderer-driver-qa4.mjs" && shasum -a 256 "$TREE/scripts/renderer-driver-qa4.mjs" "$HERE"/scene-r4.js
