/*
 * QA round 3 proxy for PR #482 (round 2's, plus: every POST /messages body is logged
 * with its text and image count, a body whose text carries HOLDSEND is held HOLD_MS
 * before it is forwarded so a conversation send stays in flight, and aside POSTs log
 * their whole text so the staged <reply-to> prefix is a wire reading).
 * Round 2 header:, on 8080 (the one port the renderer CSP allows
...
const server = createServer(async (req, res) => {
	...
	const isMessagePost = req.method === "POST" && /\/sessions\/[^/]+\/messages$/.test(url);
	let body = null;
	let entry = null;
	if (isMessagePost) {
		...
		entry = {
			...
			held: String(parsed?.text ?? "").includes("HOLDSEND") ? HOLD_MS : 0,
			status: null,
			response: null,
		};
		stats.messagePosts.push(entry);
		writeStats();
		if (entry.held) await new Promise((r) => setTimeout(r, HOLD_MS));
		entry.forwardedAt = new Date().toISOString();
	}
	// QA round 4: every non-GET call on ONE aside (DELETE = close, POST .../adopt) is
	// logged with its status, so "no adopt request left" is a wire reading.
	const isAsideOp = req.method !== "GET" && /\/asides\/[^/]+(\/adopt)?$/.test(url);
	if (isAsideOp) {
		body = await readBody(req);
		entry = {
			at: new Date().toISOString(),
			method: req.method,
			url,
			kind: /\/adopt$/.test(url) ? "adopt" : "close",
			probe: req.headers["x-qa-probe"] === "1",
			status: null,
			response: null,
		};
		stats.asideOps.push(entry);
		writeStats();
	}
	if (isAsidePost) {
		body = await readBody(req);
