...
async function sceneBtwR3(cdp) {
	...
	// QA round 4 rig fix: wait for the held POST to be ANSWERED (not just forwarded) and the
	// conversation to go idle — at load ~125 the daemon answered >2.5 s after forwarding.
	await r3AwaitRead(cdp, () => (r3msgs("HOLDSEND")[0]?.status ?? null) !== null, 300);
	await idle();
	await wait(500);
