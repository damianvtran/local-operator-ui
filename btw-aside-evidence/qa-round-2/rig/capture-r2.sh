#!/usr/bin/env bash
# QA round 1 rig for PR #482: boot the scripted provider, the REAL daemon from the
...
UI_PORT=8080
TOKEN="qa482r2-$(uuidgen | tr A-Z a-z)"
...
until mkdir "$LOCK" 2>/dev/null; do echo "port8080.lock held by a peer; retrying in 30s ($(date +%H:%M:%S))"; sleep 30; done
LOCKED=1
...
PROXY_PORT="$UI_PORT" BACKEND_HOST=127.0.0.1 BACKEND_PORT="$DESK_PORT" \
  ...
  STATS_OUT="$STATS" OLD_DAEMON="$OLD_DAEMON" \
  nohup node "$HERE/proxy-r2.mjs" > "$RUN/logs/proxy.stdout" 2>&1 &
PROXY_PID=$!
...
env -u LOCAL_OPERATOR_CONFIG_DIR node scripts/renderer-driver-qa2.mjs \
  --scene "$SCENE" \
