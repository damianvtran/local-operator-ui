#!/usr/bin/env bash
# Builds scripts/renderer-driver-qa7.mjs in the QA worktree: a COPY of the driver at
...
TREE="$1"
python3 - "$TREE/scripts/renderer-driver.mjs" "$TREE/scripts/renderer-driver-qa7.mjs" "$HERE" <<'PY'
import sys
...
for f in ("helpers-r1.js", "scene-r2.js", "scene-r3.js", "scene-r3-479.js", "scene-r4.js", "scene-r5.js", "scene-r6.js", "scene-r7.js"):
    s += "\n" + open(f"{here}/{f}").read() + "\n"
open(dst, "w").write(s)
PY
node --check "$TREE/scripts/renderer-driver-qa7.mjs" && shasum -a 256 "$TREE/scripts/renderer-driver-qa7.mjs" "$HERE"/scene-r6.js
