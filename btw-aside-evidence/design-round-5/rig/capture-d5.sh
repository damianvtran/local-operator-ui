#!/usr/bin/env bash
# Design round 5's rig (round 4's, retargeted) for PR #482: boot the scripted provider, the REAL
# daemon from the companion worktree, and my own instrumented proxy on 8080; run the scene; reap
# every listener by exact pid before this command returns.
#
#   usage: capture-d5.sh <scene> <pass-label> [window-size] [old-daemon 0|1]
#
...
#: THE SHARED 8080, taken with the shared lock (this run's coordinator's rule).
#: 8080 is also the one port `src/renderer/index.html`'s `connect-src` already
#: pins beside 1111, so the app reaches this run's proxy with NO CSP surgery.
#: The lock is checked free immediately before the bind and the rig refuses if a
#: peer's process holds the port at that moment (see the bind loop below).
UI_PORT="${UI_PORT:-8080}"
DESK_STAMP="design482r5-$(uuidgen | tr A-Z a-z)"
...
env -i HOME="$RUN" LOCAL_OPERATOR_HOME="$RUN" LOCAL_OPERATOR_CONFIG_DIR="$CONFIG" \
  ...
  nohup "$PY" "$HERE/stub-d5.py" > "$RUN/logs/stub.log" 2>&1 &
STUB_PID=$!
...
PROXY_PORT="$UI_PORT" BACKEND_HOST=127.0.0.1 BACKEND_PORT="$DESK_PORT" \
  ...
  nohup node "$HERE/r4/proxy-r4.mjs" > "$RUN/logs/proxy.stdout" 2>&1 &
PROXY_PID=$!

# A PEER RIG BINDS 8080 WITHOUT THE LOCK (round 4 measured it: another session's
# `lop serve --port 8080` took the port in the same second this rig's proxy tried
# to bind, so the proxy died with EADDRINUSE and the old single check then blamed
# "our" proxy). Retry the bind, holding the lock, until OUR pid owns the port.
TRIES=0
while :; do
  ...
  held="$(lsof -nP -iTCP:"$UI_PORT" -sTCP:LISTEN -t 2>/dev/null | head -1 || true)"
  [[ "$held" == "$PROXY_PID" ]] && break
  kill -9 "$PROXY_PID" 2>/dev/null || true
  PROXY_PID=""
  TRIES=$((TRIES + 1))
  [[ "$TRIES" -lt 80 ]] || { echo "never got 8080 after $TRIES tries" >&2; exit 3; }
  echo "a peer holds $UI_PORT (pid ${held:-none}); retrying in 30s ($(date +%H:%M:%S))"
  sleep 30
  while lsof -nP -iTCP:"$UI_PORT" -sTCP:LISTEN -t 2>/dev/null | grep -q .; do
    echo "still held by a peer; waiting 30s ($(date +%H:%M:%S))"; sleep 30
  done
  PROXY_PORT="$UI_PORT" BACKEND_HOST=127.0.0.1 BACKEND_PORT="$DESK_PORT" \
    RECORD_SRC="$RECORD" RECORDS_OUT="$RECORDS" PROXY_LOG="$RUN/logs/proxy.log" \
    STATS_OUT="$STATS" OLD_DAEMON="$OLD_DAEMON" HOLD_MS="${HOLD_MS:-10000}" \
...
env -u LOCAL_OPERATOR_CONFIG_DIR QA_PROXY_STATS="$STATS" QA_SEED_TURNS="${QA_SEED_TURNS:-0}" D5_CASES="${D5_CASES:-}" D5_THEME="${D5_THEME:-}" node scripts/renderer-driver-d5.mjs \
  --scene "$SCENE" \
