#!/usr/bin/env bash
# Builds scripts/renderer-driver-ux3.mjs (UNTRACKED) in the review worktree: a COPY of the
# driver at the PR head + rounds 1/2/3/4/5's helper layers and scenes + this round's scene,
# with the dispatch arms added. The shipped driver and the app source are not modified.
set -euo pipefail
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TREE="$1"
python3 - "$TREE/scripts/renderer-driver.mjs" "$TREE/scripts/renderer-driver-ux3.mjs" "$HERE" <<'PYEOF'
import sys
...
for f in ("helpers-r1.js", "scene-r2.js", "scene-r3.js", "scene-r3-479.js", "scene-r4.js",
          "scene-r5.js", "ux2-scene.js", "scene-ux3.js"):
    s += "\n" + open("%s/%s" % (here, f)).read() + "\n"
open(dst, "w").write(s)
PYEOF
node --check "$TREE/scripts/renderer-driver-ux3.mjs"
shasum -a 256 "$TREE/scripts/renderer-driver.mjs" "$HERE/scene-ux3.js"
