...
UI_PORT=8080
TOKEN="qa482r3-$(uuidgen | tr A-Z a-z)"
...
PROXY_PORT="$UI_PORT" BACKEND_HOST=127.0.0.1 BACKEND_PORT="$DESK_PORT" \
  ...
  nohup node "$HERE/proxy-r3.mjs" > "$RUN/logs/proxy.stdout" 2>&1 &
PROXY_PID=$!
...
env -u LOCAL_OPERATOR_CONFIG_DIR QA_PROXY_STATS="$STATS" QA_SEED_TURNS="${QA_SEED_TURNS:-0}" QA_RUNS="${QA_RUNS:-0}" QA_ASIDE_FIRST="${QA_ASIDE_FIRST:-0}" node scripts/renderer-driver-qa3.mjs \
  --scene "$SCENE" \
