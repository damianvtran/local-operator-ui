...
async function sceneBtwR2(cdp) {
	...
	const countBefore = qaCount(pageBeforeSlash, R2.UNANSWERED);
	...
	for (let i = 0; i < 300; i++) {
		...
		const page = await r2PageText(cdp);
		if (qaCount(page, R2.UNANSWERED) > countBefore) {
			slashNote = { ms: Date.now() - t1, inTranscript: qaCount(band.transcriptText, R2.UNANSWERED), inBand: (band.bandNotice ?? "").includes(R2.UNANSWERED), panel: band.panel };
			break;
		}
		await wait(100);
	}
	...
	r2check(
		...
		slashNote !== null && slashNote.inTranscript >= 1 && afterSlash.panel === null,
	...
	r2check(
		...
		qaCount(await r2PageText(cdp), R2.UNANSWERED) === countBefore + 1,
		`occurrences ${countBefore} -> ${qaCount(await r2PageText(cdp), R2.UNANSWERED)}`,
	);
	...
	const countBefore2 = qaCount(await r2PageText(cdp), R2.UNANSWERED);
	await clickAt(cdp, R2.FIELD);
	...
	for (let i = 0; i < 300; i++) {
		const band = await readQaBand(cdp);
		const page = await r2PageText(cdp);
		if (qaCount(page, R2.UNANSWERED) > countBefore2) {
			compNote = { ms: Date.now() - t2, band: band.bandNotice, inTranscript: qaCount(band.transcriptText, R2.UNANSWERED) - qaCount(gone2.transcriptText, R2.UNANSWERED) };
			break;
		}
		await wait(100);
	}
	...
	r2check(
		...
		compNote !== null && (compNote.band ?? "").includes(R2.UNANSWERED),
		JSON.stringify(compNote),
	);
	await wait(700);
	r2check(
		"(c) composer door: the refusal is stated exactly once (not also as a transcript note)",
		qaCount(await r2PageText(cdp), R2.UNANSWERED) === countBefore2 + 1,
		`occurrences ${countBefore2} -> ${qaCount(await r2PageText(cdp), R2.UNANSWERED)}`,
	);
