#!/usr/bin/env bash
# UX round 5 rig (from rounds 1-4's) for PR #482: boot the scripted provider, the REAL daemon from the
# companion worktree, and my own instrumented proxy on a PRIVATE port; run the UX scene; reap
# every listener by exact pid before this command returns.
#
#   usage: capture-ux5.sh <scene> <pass-label> [window-size] [old-daemon 0|1]
#
# WHY A PRIVATE PORT AND NOT 8080. Round 4's rig used 8080, the one port the shipped
# `connect-src` already allows beside 1111. On this host at this hour 8080 is held by the
# operator's OWN app (`ps` -> `Local Operator [serve] port=1111 ... --listener-fd 3`, which
# also listens on 8080), so waiting for it would never end and signalling it is out of the
# question. This rig therefore binds 8391, a port nothing on this host listens on, and the
# built page's `connect-src` is widened for it in the GITIGNORED build output only
# (`out/renderer/index.html`, see widen-csp-ux5.sh); no src/ file is touched. The shared
# 8080 lock is deliberately NOT taken, because this rig never touches 8080 and holding it
# would block peers who do.
#
# Hard rules this script keeps: 1111 is never bound (it is the operator's own
# backend); the stub and the daemon bind OS-chosen ports; the private port is checked free
...
UI_PORT=8391
DESK_STAMP="ux482r5-$(uuidgen | tr A-Z a-z)"
...
reap() {
  ...
  if [[ -n "$PROXY_PID" ]]; then
    ...
    if [[ -n "$held" ]]; then
      ...
      echo "$UI_PORT released before this command returned"
    fi
  fi
  if [[ "$LOCKED" == 1 ]]; then rmdir "$LOCK" && echo "ux5 private-port lock released"; fi
...
# The private port must be free and nothing else may take it while this pass runs. The lock is
# this rig's own (a peer cannot be told to avoid 8391), so the bind check below is the guard
# that matters: the port is refused unless the process holding it afterwards is OUR proxy.
for _ in $(seq 1 120); do
  if ! lsof -nP -iTCP:"$UI_PORT" -sTCP:LISTEN -t >/dev/null 2>&1 && mkdir "$LOCK" 2>/dev/null; then GOT=1; break; fi
  echo "$UI_PORT or its lock is held; retrying in 30s ($(date +%H:%M:%S))"; sleep 30
done
[[ "${GOT:-0}" == 1 ]] || { echo "never got $UI_PORT" >&2; exit 3; }
LOCKED=1
echo "ux5 private-port lock taken"
if lsof -nP -iTCP:"$UI_PORT" -sTCP:LISTEN -t 2>/dev/null | grep -q .; then
  echo "$UI_PORT became occupied between the check and the bind; refusing" >&2
  exit 3
fi
PROXY_PORT="$UI_PORT" BACKEND_HOST=127.0.0.1 BACKEND_PORT="$DESK_PORT" \
  ...
  nohup node "$HERE/proxy-r4.mjs" > "$RUN/logs/proxy.stdout" 2>&1 &
PROXY_PID=$!
...
[[ "$(lsof -nP -iTCP:"$UI_PORT" -sTCP:LISTEN -t 2>/dev/null | head -1)" == "$PROXY_PID" ]] || {
  echo "$UI_PORT is not held by our proxy ($PROXY_PID); refusing" >&2; exit 3; }
curl -fsS "http://127.0.0.1:$UI_PORT/health" >/dev/null || { echo "the proxy never answered /health" >&2; exit 1; }
echo "rig up: proxy $PROXY_PID on $UI_PORT -> daemon $DESK_PID on $DESK_PORT, stub $STUB_PORT; 1111 and 8080 untouched"

# --- the scene ---------------------------------------------------------------
export LOCAL_OPERATOR_DESKTOP_TOKEN="$DESK_STAMP"
cd "$TREE" || exit 1
echo "== $PASS ($SCENE) at $(date -u +%H:%M:%S) =="
env -u LOCAL_OPERATOR_CONFIG_DIR QA_PROXY_STATS="$STATS" QA_SEED_TURNS="${QA_SEED_TURNS:-0}" QA_RUNS="${QA_RUNS:-0}" QA_ASIDE_FIRST="${QA_ASIDE_FIRST:-0}" QA_CAP_CASES="${QA_CAP_CASES:-}" node scripts/renderer-driver-ux5.mjs \
  --scene "$SCENE" \
