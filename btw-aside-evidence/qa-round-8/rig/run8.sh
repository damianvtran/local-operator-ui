...
env -i HOME=$R/home PATH="$PATH" TERM=xterm-256color GIT_CONFIG_SYSTEM=/dev/null GIT_TERMINAL_PROMPT=0 TMPDIR=$R/tmp \
  ...
  QA_CAP_CASES="${QA_CAP_CASES:-}" QA_SEED_TURNS="${QA_SEED_TURNS:-0}" QA_RUNS="${QA_RUNS:-0}" QA_ASIDE_FIRST="${QA_ASIDE_FIRST:-0}" QA_BAND_SHORT_CWD="${QA_BAND_SHORT_CWD:-$HOME/tools}" \
  QA_UI_TREE=/Users/damian/local-operator-ui-worktrees/qa-482-r8 \
  QA_BACKEND_TREE=/Users/damian/local-operator-worktrees/qa482-r8 \
  bash -c 'mkdir -p "$HOME" "$TMPDIR"; bash ./capture-r8.sh "$@"' _ "$@" > "$2.out" 2>&1
echo "exit $?"
