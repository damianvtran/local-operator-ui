#!/usr/bin/env bash
# QA round 8 rig (round 7's, re-pointed) for PR #482: boot the scripted provider, the REAL daemon from the
...
UI_PORT=8080
DESK_STAMP="qa482r8-$(uuidgen | tr A-Z a-z)"
...
printf '# workspace\n\nZZLIVEBODYZZ scratch readme for the fold probe.\n' > "$WORKSPACE/README.md"
printf '# workspace\n\nZZLIVEBODYZZ scratch readme for the fold probe.\n' > "$RUN/README.md"
cd "$RUN" || exit 1
...
env -u LOCAL_OPERATOR_CONFIG_DIR QA_PROXY_STATS="$STATS" QA_SEED_TURNS="${QA_SEED_TURNS:-0}" QA_RUNS="${QA_RUNS:-0}" QA_ASIDE_FIRST="${QA_ASIDE_FIRST:-0}" QA_CAP_CASES="${QA_CAP_CASES:-}" node scripts/renderer-driver-qa8.mjs \
  --scene "$SCENE" \
