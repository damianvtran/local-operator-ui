...
try {
	...
	/*
	 * QA r6 addition, additive only: the ids of every row mounted at settle, BEFORE
	 * any scrolling, and every mounted tool row still holding an output stand-in.
	 * "The row kept its stand-in" and "the row is not painted at all" are different
	 * user-visible symptoms, and the probe's scroll sweeps the virtualized window
	 * upwards, so only this pre-scroll dump can tell them apart.
	 */
	summary.domAtSettle = JSON.parse(await evaluate(`JSON.stringify({ ids: [...document.querySelectorAll('[data-record-id]')].map((e) => e.getAttribute('data-record-id')), standIns: [...document.querySelectorAll('[data-record-kind="tool"]')].map((el) => { const c = el.querySelectorAll("span.truncate"); const s = c[1]?.textContent ?? ""; return s.startsWith("\\u2026 ") ? { id: el.getAttribute("data-record-id"), summary: s.slice(0, 60) } : null; }).filter(Boolean) })`));
	...
	/*
	 * Round 7 (R18). A SECOND PASS over the same conversation. The waiting set is
	 * per conversation and OUTLIVES the seed that filled it, so a call the first
	 * pass announced as still waiting at a gate stays exempt from the floor unless
	 * the next pass's seed retracts it - and then the walk stops one page short and
	 * the call keeps no label. Two deliveries, because they are different user
	 * paths: a real switch away and back, and a re-attach (a second snapshot for
	 * the same conversation on the stream that is already open). The row records
	 * which one ran, whether a second seed arrived at all, and the read log split at
	 * the pass boundary.
	 */
	if (NAME === "r7-second-pass" || NAME === "r7-reattach") {
		const other = Object.keys(scen.sessions).find((k) => k !== scen.open);
		await wait(1500);
		const p1 = await read();
		summary.pass1 = { ...tally(p1), reads: log.history.length, streams: log.streams.filter((x) => x.sid === scen.open).length };
		mark("pass1-settled");
		/*
		 * From here the conversation's seed is the SECOND pass's. `frontend(sid)`
		 * reads `live_events` at subscription time, so a re-subscribe and the
		 * re-attach frame below both serve the new set.
		 */
		scen.sessions[scen.open].live_events = scen.pass2.live_events;
		const readsBefore = log.history.length;
		const streamsBefore = log.streams.filter((x) => x.sid === scen.open).length;
		if (NAME === "r7-second-pass") {
			await evaluate(`(() => { location.hash = "#/chat/${other}"; return 1; })()`);
			await wait(1200);
			mark("away");
			await evaluate(`(() => { location.hash = "#/chat/${scen.open}"; return 1; })()`);
			...
			const s = scen.sessions[scen.open];
			const stream = state.streams[scen.open];
			if (!stream) summary.reattachNoStream = true;
			else
				stream.send({ type: "snapshot", payload: {
		...
		// Bounded wait for the second pass to settle: it ends as soon as the reads
		// this pass asked for have been answered.
		const deadline2 = now() + 12000;
		while (now() < deadline2) {
			await wait(200);
			const h = log.history.slice(readsBefore);
			if (h.length > 0 && h.every((x) => x.answeredAt !== undefined)) break;
			if (now() - log.t0 > 3000 && log.history.length === readsBefore) break;
		}
		await wait(1500);
		summary.pass2 = {
			reads: log.history.slice(readsBefore),
			streamsBefore,
			streamsAfter: log.streams.filter((x) => x.sid === scen.open).length,
		};
		const p2 = await read();
		summary.pass2Settled = { ...tally(p2), readsAfter: log.history.length - readsBefore };
		summary.pass2Mounted = JSON.parse(await evaluate(`JSON.stringify([...document.querySelectorAll('[data-record-id]')].map((e) => e.getAttribute('data-record-id')).filter((x) => ${JSON.stringify(scen.probe ?? [])}.some((id) => x.includes(id))))`));
		await capture("05-second-pass");
	}
	...
	if (Array.isArray(scen.probe)) {
		...
		summary.probe = got;
		/*
		 * QA r6 addition, additive only: `mounted: false` says the row is not under
		 * `data-record-id="tool:<id>"`, which does not by itself say whether the call is
		 * painted under some other record id (a seeded row) or not painted at all. This
		 * scans every mounted record id for the probe's call id, and lists every object
		 * column still holding an output stand-in, so the "kept its stand-in" half of a
		 * claim is measured rather than inferred.
		 */
		summary.probeAnyId = JSON.parse(await evaluate(`JSON.stringify(${JSON.stringify(scen.probe)}.map((id) => ({ id, ids: [...document.querySelectorAll('[data-record-id]')].map((e) => e.getAttribute('data-record-id')).filter((x) => x.includes(id)) })))`));
		summary.standInsAll = JSON.parse(await evaluate(`JSON.stringify([...document.querySelectorAll('[data-record-kind="tool"]')].map((el) => { const c = el.querySelectorAll("span.truncate"); const s = c[1]?.textContent ?? ""; return s.startsWith("\u2026 ") ? { id: el.getAttribute("data-record-id"), name: c[0]?.textContent ?? "", summary: s.slice(0, 70) } : null; }).filter(Boolean))`));
