...
/*
 * QA round 2, PR 511. One 3 MB transcript between every scenario, by reference:
 * the volume is near-full and nine copies of the same journal buy nothing.
 * Nothing else about how a session is served changes.
 */
for (const s of Object.values(scen.sessions ?? {})) {
	if (s.journal_file) s.journal = JSON.parse(readFileSync(s.journal_file, "utf8"));
}
...
const now = () => Date.now();
const log = { t0: now(), history: [], unknown: [], streams: [], asides: [] };
...
const server = createServer((req, res) => {
	...
	/*
	 * QA round 2, row 5: the slash catalogue. The composer's commands come from the
	 * BACKEND (`/v1/desktop/commands`), so without this route `/btw` is simply not a
	 * command and #482's panel is unreachable from the chat surface. The entry is the
	 * backend registry's own row (`local_operator/slash_commands.py`,
	 * `desktop_destination="session.aside"`), in the shape the renderer's
	 * `SlashCommandMeta` reads.
	 */
	if (p === "/v1/desktop/commands")
		return json(200, { status: 200, result: { commands: scen.commands ?? [
			{ name: "btw", description: "Ask a side question off the record (esc closes it)", aliases: [], arguments: "none", echo: false, consumes_prompt: true, prefixes_text: true, argument_shape: "any", argument_words: [], destination: "session.aside", execution: "native" },
		] } });
	...
	if (em && scen.sessions[em[1]]) {
		...
		send({ type: "open", payload: { subscription_id: "a1b2c3d4e5f60718293a4b5c6d7e8f90", gap: false, watch_ttl_seconds: 45 } });
	...
	/*
	 * QA round 2, row 5: the `/btw` aside's own POST. The answer's shape is the
	 * backend's (`local_operator/server/routes/desktop_lifecycle.py`: `{"data":
	 * {"aside_id": body.request_id, "text": answer, "off_record": True}}`,
	 * unwrapped by `desktopResult`), and the live `aside_delta` frames are the
	 * runtime's - this rig pushes those itself.
	 */
	const asideAsk = p.match(/^\/v1\/desktop\/sessions\/([^/]+)\/asides$/);
	if (asideAsk && req.method === "POST") {
		let body = "";
		req.on("data", (d) => { body += d; });
		req.on("end", () => {
			let parsed = {};
			try { parsed = JSON.parse(body); } catch {}
			log.asides.push({ at: now() - log.t0, sid: asideAsk[1], body: parsed });
			setTimeout(() => json(200, { status: 200, result: { data: { aside_id: parsed.request_id ?? "aside-qa", text: scen.aside_answer ?? "QA stub: the aside's own answer text.", off_record: true } } }), Number(scen.aside_delay ?? 150));
		});
		return;
	}
	if (/^\/v1\/desktop\/sessions\/[^/]+\/asides\/[^/]+$/.test(p)) {
		log.unknown.push(`${req.method} ${p} (aside get/close, answered)`);
		return json(200, { status: 200, result: { data: { aside_id: p.split("/").pop(), status: req.method === "DELETE" ? "closed" : "open" } } });
	}
...
/* QA round 2, row 1: the published still is the ROW, not the pane. CDP's `clip` takes
 * CSS pixels, so the crop is the row's own rect and needs no image arithmetic. */
const captureClip = async (name, clip) => {
...
/* ---------------------------------------------------------------------------
 * QA round 2 additions (PR 511, round 2 of the QA pass).
 *
 * `pushRaw` is the rig's own way to hand the app any session frame: the durable
 * round ending a retry hangs on (`{type: "event", payload: {type: "turn_end"}}`)
 * and the re-sync snapshot a degraded canonical delta produces. `commitRows`
 * appends the STEP's rows to the served journal - which is what "the step
 * commits" means on the wire: the assistant row that carries the call's
 * arguments, and its result row, appear in the journal the app reads back.
 * --------------------------------------------------------------------------- */
const pushRaw = async (frame) => { state.streams[scen.open].send(frame); return frame; };
const commitRows = () => {
	const s = scen.sessions[scen.open];
	for (const item of [...(scen.commit?.insert ?? [])].sort((a, b) => a.index - b.index))
		s.journal.splice(item.index, 0, item.row);
};
const pushSnapshot = async () => {
	const s = scen.sessions[scen.open];
	if (scen.commit?.resync_live_events) s.live_events = scen.commit.resync_live_events;
	const payload = {
		frontend: frontend(scen.open),
		history: { entries: s.journal.slice(-s.page_n), has_more: s.journal.length > s.page_n, cursor_missing: false },
		cold: false, cold_reason: null, attaching: false,
	};
	await evaluate(`(() => { window.__qaPushes = window.__qaPushes || []; window.__qaPushes.push({ at: performance.now(), id: "resync-snapshot" }); return 1; })()`);
	return pushRaw({ type: "snapshot", payload });
};
...
try {
	...
	if (SETTLES.length > 0 && !["r1c", "r3c"].includes(ROW)) {
		...
		/*
		 * QA round 2 addition, row 1: the row AT ITS OWN POSITION, brought into the
		 * viewport and photographed, so the published still shows the object column
		 * rather than whichever rows happened to be on screen. No read of its own.
		 */
		if (ROW === "r1") {
			summary.rowInView = [];
			for (const id of SETTLES) {
				const view = JSON.parse(await evaluate(`(() => {
					const all = [...document.querySelectorAll('[data-record-id]')];
					const el = document.querySelector('[data-record-id="tool:' + ${JSON.stringify(id)} + '"]') ?? all.find((e) => (e.getAttribute("data-record-id") ?? "").includes(${JSON.stringify(id)}));
					if (!el) return JSON.stringify({ mounted: false });
					el.scrollIntoView({ block: "center" });
					const r = el.getBoundingClientRect();
					const cells = el.querySelectorAll("span.truncate");
					const summary = cells[1]?.textContent ?? "";
					return JSON.stringify({ mounted: true, top: Math.round(r.top), left: Math.round(r.left), width: Math.round(r.width), height: Math.round(r.height), name: cells[0]?.textContent ?? "",
						summary: summary.slice(0, 120), standIn: summary.startsWith("\\u2026 "), labelled: summary !== "" && !summary.startsWith("\\u2026 "), text: el.innerText.slice(0, 200) });
				})()`));
				summary.rowInView.push(view);
				if (view.mounted) {
					/* The scroll has to have painted before the clip is taken, or the crop
					 * catches the row where it used to be. */
					await wait(350);
					await captureClip(`05-row-in-view-clip`, { x: Math.max(0, view.left), y: Math.max(0, view.top - 46), width: Math.min(view.width, 900), height: view.height + 92 });
				}
			}
			await wait(400);
			await capture("05-row-in-view");
		}
	}

	/* ---------------------------------------------------------------------------
	 * QA round 2, rows 1c and 3c: THE PHASED SETTLE.
	 *
	 * One settle, then the two moments the round-2 review turns on:
	 *   settle  - the call's step is still open, so no durable row carries its
	 *             arguments: the read is speculative and must not cost the call its
	 *             retry (R1), and must be bounded to one tail page when the frame
	 *             states the call's own start (R2);
	 *   resync  - a re-sync snapshot whose seed names the same call, which is the
	 *             second spend the pre-remediation head made before the row existed;
	 *   commit  - the step's rows appear in the journal and a durable round ending
	 *             (`turn_end`) arrives - the moment a retry can succeed.
	 *
	 * Every phase records the requests it caused (with the page each answered), the
	 * ROWS they delivered - the spend R2's bound is stated in - and the row's
	 * painted state at its end. The row is never scrolled to or re-opened.
	 * ------------------------------------------------------------------------- */
	if (ROW === "r1c" || ROW === "r3c") {
		const id = SETTLES[0];
		const phaseMs = Number(scen.phase_ms ?? 3500);
		await evaluate(SAMPLER_TARGETS([id], Math.max(24000, Number(scen.sample_ms ?? 24000))));
		summary.phases = [];
		const phase = async (name, fn) => {
			const before = log.history.length;
			if (fn) await fn();
			const until = now() + phaseMs;
			while (now() < until) await wait(150);
			const reads = readsFrom(before);
			const row = await probeRow(id);
			const p = {
				phase: name,
				at: now() - log.t0,
				requests: reads.length,
				rowsSpent: reads.reduce((n, r) => n + (r.rows ?? 0), 0),
				reads,
				row: { mounted: row.mounted, labelled: row.labelled, standIn: row.standIn, blank: row.blank, name: row.name, summary: (row.summary ?? "").slice(0, 80) },
			};
			summary.phases.push(p);
			return p;
		};
		summary.preSettleRow = await probeRow(id);
		await phase("settle", () => pushSettle(id));
		await capture("03-after-settle");
		if (scen.commit?.resync_snapshot) {
			await phase("resync", () => pushSnapshot());
			await capture("04-after-resync");
		}
		if (scen.commit?.insert) {
			await phase("commit", async () => { commitRows(); await pushRaw({ type: "event", payload: { type: scen.commit.turn_end ?? "turn_end" } }); });
			await capture("05-after-commit");
		}
		const frames = JSON.parse(await evaluate("JSON.stringify(window.__qaLive)"));
		const pushes = JSON.parse(await evaluate("JSON.stringify(window.__qaPushes)"));
		const push = pushes.find((p) => p.id === id);
		summary.latency = push
			? (() => { const after = frames.filter((f) => f[0] >= push.at); const lab = after.find((f) => f[1] === 3); const stand = after.find((f) => f[1] >= 2); return { msToStandIn: stand ? stand[0] - push.at : null, msToLabelled: lab ? lab[0] - push.at : null, framesSampled: after.length }; })()
			: { push: null };
		summary.settleFrames = frameRunsLike(frames, 1);
		summary.postPhasesRow = await probeRow(id);
		summary.standInsAfter = JSON.parse(
			await evaluate(
				`JSON.stringify([...document.querySelectorAll('[data-record-kind="tool"]')].map((el) => { const c = el.querySelectorAll("span.truncate"); const s = c[1]?.textContent ?? ""; return s.startsWith("\\u2026 ") ? { id: el.getAttribute("data-record-id"), summary: s.slice(0, 60) } : null; }).filter(Boolean))`,
			),
		);
		summary.readsTotal = log.history.length;
		summary.rowsSpentTotal = log.history.reduce((n, h) => n + (h.rows ?? 0), 0);
	}
	...
	/* ---------------------------------------------------------------------------
	 * QA round 2, row 5: the FOLD. The folded head is #482's tree plus this
	 * branch's two manifest keys, so this row walks the chat surface the fold
	 * touched: the transcript paints, and `/btw` reaches the aside panel that
	 * #482 put above the composer (a bare `/btw` attaches an EMPTY panel and asks
	 * nothing, so the panel is reachable with no wire call at all).
	 * ------------------------------------------------------------------------- */
	if (ROW === "r5") {
		const aside = {};
		/* The panel is the one `<section>` whose heading is `Aside` (aside-panel.tsx
		 * renders `<h2 id={titleId}>Aside</h2>` and names the section by it). */
		const PANEL = `[...document.querySelectorAll("section")].find((s) => s.querySelector("h2")?.textContent?.trim() === "Aside")`;
		const panelState = async () => JSON.parse(await evaluate(`(() => { const el = ${PANEL}; return JSON.stringify({ panel: !!el, text: (el?.innerText ?? "").slice(0, 400), draft: document.querySelector("textarea")?.value ?? null, dialogs: document.querySelectorAll("[role=dialog]").length, composer: !!document.querySelector("textarea"), composerDisabled: document.querySelector("textarea")?.disabled ?? null }); })()`));
		aside.transcript = await read().then((snap) => ({ ...tally(snap), rows: snap.rows.length }));
		const press = async (keyName, code, virtualKeyCode) => {
			for (const type of ["keyDown", "keyUp"])
				await send("Input.dispatchKeyEvent", { type, key: keyName, code, windowsVirtualKeyCode: virtualKeyCode, nativeVirtualKeyCode: virtualKeyCode });
		};
		const box = JSON.parse(await evaluate(`(() => { const t = document.querySelector("textarea"); if (!t) return "null"; const r = t.getBoundingClientRect(); return JSON.stringify({ x: r.left + r.width / 2, y: r.top + r.height / 2 }); })()`));
		if (!box) { aside.error = "no composer textarea"; } else {
			for (const type of ["mousePressed", "mouseReleased"])
				await send("Input.dispatchMouseEvent", { type, x: box.x, y: box.y, button: "left", clickCount: 1 });
			await wait(300);
			await send("Input.insertText", { text: "/btw" });
			await wait(700);
			aside.beforeBareBtw = await panelState();
			aside.draftIsBareBtw = await evaluate(`document.querySelector("textarea")?.value ?? null`);
			await press("Enter", "Enter", 13);
			await wait(1400);
			aside.afterBareBtw = await panelState();
			aside.readsFromBareBtw = log.history.length - (aside.readsAtOpen ?? 0);
			aside.asksFromBareBtw = log.asides.length;
			await capture("07-aside-bare");
			/* The question. `Input.insertText` because the composer is controlled (a DOM
			 * write fires no input event, so React never sees the draft). */
			await send("Input.insertText", { text: "QA round 2: does the panel stream?" });
			await wait(400);
			const beforeAsk = log.history.length;
			await press("Enter", "Enter", 13);
			/* It is the POST that mints the aside id, so the live chunks are pushed once
			 * the stub has it - the runtime's own shape, after the request is in flight. */
			let ask = null;
			for (let i = 0; i < 30 && !ask; i++) { await wait(100); ask = log.asides.at(-1) ?? null; }
			aside.ask = ask ? { at: ask.at, text: ask.body.text, request_id: ask.body.request_id, subscription_id: ask.body.subscription_id ?? null, aside_id: ask.body.aside_id ?? null } : null;
			if (ask) {
				await pushRaw({ type: "aside_delta", payload: { aside_id: ask.body.request_id, delta: "QA chunk one: " } });
				await wait(500);
				aside.midAsk = await panelState();
				await pushRaw({ type: "aside_delta", payload: { aside_id: ask.body.request_id, delta: "and chunk two." } });
			}
			await wait(3000);
			aside.afterAsk = await panelState();
			aside.readsDuringAside = log.history.length - beforeAsk;
			aside.posts = log.asides.map((a) => ({ at: a.at, text: a.body.text, request_id: a.body.request_id, subscription_id: a.body.subscription_id ?? null }));
			aside.transcriptAfterAside = await read().then((snap) => ({ ...tally(snap), rows: snap.rows.length }));
			await capture("08-aside-answered");
		}
		summary.aside = aside;
	}

	summary.unknownRoutes = [...new Set(log.unknown)];
	summary.asideRoutes = log.asides.length;
