...
if (seedArgument < 0)
	throw new Error(
		"Pass --seed <complete signed Python root>; never an installed app to modify",
	);
...
let installs = 0;
const selection = await runtime.prepareManagedPython(
	options,
	async (venv, python) => {
		installs++;
		...
		const installEnv = {
			...
			LOCAL_OPERATOR_SUPPORT_PATH: support,
		...
		try {
			const result = await execute(
				...
				{ env: installEnv, timeout: 900_000, maxBuffer: 32 * 1024 * 1024 },
			);
			writeFileSync(
				join(scratch, "install.log"),
...
assert.equal(
	runtime.runtimeId(runtime.runtimeManifest(seed, process.arch)),
	before,
);
assert.equal(
	readFileSync(join(legacy, "user-owned.txt"), "utf8"),
	"preserve legacy bytes\n",
);
assert.equal(await runtime.managedSelectionReady(options), true);
const repeated = await Promise.all([
	runtime.prepareManagedPython(options, () => {
		throw new Error("Reuse must not reinstall");
	}),
	runtime.prepareManagedPython(options, () => {
		throw new Error("Concurrent reuse must not reinstall");
	}),
]);
assert.deepEqual(repeated, [selection, selection]);
renameSync(app, join(scratch, "source-namespace-unavailable"));
const probe =
	"import os,sys,sysconfig,encodings,webbrowser,_ssl,_sqlite3,json; p=[sys._base_executable,sys.base_prefix,sysconfig.get_path('stdlib'),encodings.__file__,webbrowser.__file__]+sys.path; assert all('.app/' not in os.path.realpath(x).lower() for x in p if x),p; print(json.dumps(p))";
for (const flag of ["-E", "-I"]) {
	const response = await execute(
		join(selection.venv, "bin", "python"),
		[flag, "-c", probe],
		{ env, timeout: 30_000 },
	);
...
assert.equal(await runtime.managedSelectionReady(options), true);
writeFileSync(
	join(scratch, "selection.json"),
	JSON.stringify(selection, null, 2),
);
console.log(
	"PASS actual shipped install; backend health; selected final paths; source namespace unavailable; -E/-I imports; cache-tolerant signed reuse; concurrent reuse; legacy bytes preserved",
);
