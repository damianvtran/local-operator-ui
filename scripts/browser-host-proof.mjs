...
/**
 * The renderer debugging port. Randomised per run rather than fixed: a fixed port
 * is how an earlier failed run's leftover app answered this run's CDP calls, which
 * looked exactly like "the IPC handler is missing" — the wrongest possible reading
 * of a stale process.
 */
const DEVTOOLS_PORT = 9200 + Math.floor(Math.random() * 600);
 ...
 * uncaught exception, a permission request, a popup attempt, and a same-origin
 * link so a click can be seen to navigate.
 */
...
/** Fail fast if something else already owns the debugging port: see the constant. */
async function assertDevtoolsPortFree() {
	try {
		const response = await fetch(`http://127.0.0.1:${DEVTOOLS_PORT}/json/version`);
		if (response.ok) {
			throw new Error(
				`port ${DEVTOOLS_PORT} is already serving a debugger — a leftover app would answer this run's CDP calls`,
			);
		}
	} catch (error) {
