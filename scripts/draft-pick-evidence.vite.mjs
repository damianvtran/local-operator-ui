...
/**
 * Vite config for the draft-pick evidence harness only.
 *
 * A SIBLING OF `submit-latency-evidence.vite.mjs`, and it carries the proxy for
 * the same reason: the claim under the camera is the round trip, so the shipped
 * `desktopRequest` takes its same-origin `/__desktop` branch against a real,
 * isolated `local-operator serve`. The token and the backend URL stay in THIS
 * Node process.
 *
 * Two instances run beside each other, which is why the port is read from the
 * environment with the same 5204 default this file had hard-coded: the negative
 * control needs the SAME page against a backend WITHOUT `draft_selection`, and
 * the two must be one config rather than one file per answer.
 *
 ...
 *
 * Port 5204 by default, distinct from its siblings (5199 backend-error, 5201
 * usage-real, 5202 submit-latency, 5203 its before column), so several harnesses
 * can run at once.
 */
...
export default defineConfig({
	...
	server: {
		port: Number(process.env.DRAFT_PICK_EVIDENCE_PORT ?? 5204),
		strictPort: true,
		/*
