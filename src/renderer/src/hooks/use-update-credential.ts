...
import type { CredentialUpdate, CredentialListResult } from '@renderer/api/local-operator/types';
...
export const useUpdateCredential = () => {
  ...
  return useMutation({
    mutationFn: async (credentialUpdate: CredentialUpdate) => {
      try {
        const response = await client.credentials.updateCredential(credentialUpdate);
    ...
    onSuccess: async (_data, variables) => {
      ...
      queryClient.setQueryData<CredentialListResult | null>(credentialsQueryKey, (oldData) => {
