...
import type { AgentUpdate, AgentDetails } from '@renderer/api/local-operator/types';
...
export const useUpdateAgent = () => {
  ...
  return useMutation({
    mutationFn: async ({ agentId, update }: { agentId: string; update: AgentUpdate }) => {
      try {
    ...
    onSuccess: async (_data, { agentId }) => {
      ...
      queryClient.setQueryData<AgentDetails | null>([...agentsQueryKey, agentId], (oldData) => {
