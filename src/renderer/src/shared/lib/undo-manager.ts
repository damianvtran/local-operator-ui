...
export class UndoManager {
	...
	private addState(content: string): void {
		...
		if (
			...
			this.history[this.pointer]?.content === content
		) {
			return;
		}
	...
	private restoreSelection(selectionState?: HistoryState["selection"]): void {
		...
		try {
			const startContainer = this.getNodeFromPath(
	 ...
	 */
	private restoreSelectionFallback(
		selectionState: HistoryState["selection"],
	): void {
		if (!selectionState) {
