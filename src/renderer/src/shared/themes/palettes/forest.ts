...
export const forest: ThemeDefinition = {
	...
	palette: {
		...
		/*
		 * The current row's own ground: `surface` stepped 5.06 `L*` up at the panel's own
		 * hue (1.1 degrees off, inside the 12-degree bound) and carried
		 * 9.14 `C*` against the panel's 8.31 — the panel's own colour, one step lighter, which
		 * is what the operator asked for. ΔE00 from `surface` 3.42, from
		 * `elevated` 0.70, from `sunken` 9.60. Ink on this ground: `ink` 11.27:1,
		 * `ink-muted` 7.00:1, `ink-dim` 4.66:1 — `ink-dim` is the binder, and
		 * the 0.15 of headroom it keeps is the floor this mark is authored against.
		 *
		 * WHAT THIS REPLACES: the value before this round — `1.39x` the panel's
		 * chroma (11.57 `C*` against 8.31), 7 degrees off its hue, ΔE00 4.19 from
		 ...
		 *
		 * THE HOVER STEP IS THE COLLISION: 0.70 to `elevated` is under the field floor,
