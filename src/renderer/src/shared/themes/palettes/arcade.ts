...
export const arcade: ThemeDefinition = {
	...
	palette: {
		...
		/*
		 * The current row's own ground: `surface` stepped 4.00 `L*` up at the panel's own
		 * hue (1.0 degrees off, inside the 12-degree bound) and carried
		 * 2.94 `C*` against the panel's 2.09 — the panel's own colour, one step lighter, which
		 * is what the operator asked for. ΔE00 from `surface` 2.61, from
		 * `elevated` 0.31, from `sunken` 6.59. Ink on this ground: `ink` 13.83:1,
		 * `ink-muted` 7.81:1, `ink-dim` 4.65:1 — `ink-dim` is the binder, and
		 * the 0.15 of headroom it keeps is the floor this mark is authored against.
		 *
		 * WHAT THIS REPLACES: the value before this round — `0.75x` the panel's
		 * chroma (1.56 `C*` against 2.09), 160 degrees off its hue, ΔE00 4.14 from
		 ...
		 *
		 * THE HOVER STEP IS THE COLLISION: 0.31 to `elevated` is under the field floor,
