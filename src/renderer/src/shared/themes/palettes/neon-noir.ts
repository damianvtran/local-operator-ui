...
export const neonNoir: ThemeDefinition = {
	...
	palette: {
		...
		/*
		 * The current row's own ground: `surface` stepped 3.96 `L*` up at the panel's own
		 * hue (3.5 degrees off, inside the 12-degree bound) and carried
		 * 6.42 `C*` against the panel's 5.23 — the panel's own colour, one step lighter, which
		 * is what the operator asked for. ΔE00 from `surface` 2.80, from
		 * `elevated` 1.08, from `sunken` 7.23. Ink on this ground: `ink` 11.16:1,
		 * `ink-muted` 6.54:1, `ink-dim` 4.69:1 — `ink-dim` is the binder, and
		 * the 0.15 of headroom it keeps is the floor this mark is authored against.
		 *
		 * WHAT THIS REPLACES: the value before this round — `1.18x` the panel's
		 * chroma (6.17 `C*` against 5.23), 24 degrees off its hue, ΔE00 4.03 from
		 ...
		 *
		 * THE HOVER STEP IS THE COLLISION: 1.08 to `elevated` is under the field floor,
