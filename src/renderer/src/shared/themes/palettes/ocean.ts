...
export const ocean: ThemeDefinition = {
	...
	palette: {
		...
		/*
		 * The current row's own ground: `surface` stepped 4.64 `L*` up at the panel's own
		 * hue (2.1 degrees off, inside the 12-degree bound) and carried
		 * 11.09 `C*` against the panel's 9.90 — the panel's own colour, one step lighter, which
		 * is what the operator asked for. ΔE00 from `surface` 3.21, from
		 * `elevated` 0.82, from `sunken` 9.94. Ink on this ground: `ink` 11.18:1,
		 * `ink-muted` 7.01:1, `ink-dim` 4.65:1 — `ink-dim` is the binder, and
		 * the 0.15 of headroom it keeps is the floor this mark is authored against.
		 *
		 * WHAT THIS REPLACES: the value before this round — `1.05x` the panel's
		 * chroma (10.40 `C*` against 9.90), 14 degrees off its hue, ΔE00 4.01 from
		 ...
		 *
		 * THE HOVER STEP IS THE COLLISION: 0.82 to `elevated` is under the field floor,
