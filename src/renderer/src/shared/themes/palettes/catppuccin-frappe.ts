...
export const catppuccinFrappe: ThemeDefinition = {
	...
	palette: {
		...
		// The user block's fill: ΔE00 4.26 off the canvas, where `surface` alone
		// measures 2.39 - a step toward `elevated` to clear the role's 4.0 floor,
		// which the block carries because the fill IS its boundary.
		messageSurface: "#3b4153",
		/*
		 ...
		 *
		 * rowHover    #3E404C  panel hue, C* 7.83,
		 *                       the rule's 0.60 x the panel's 12.60; +2.19 L*,
		 *                       ΔE00 3.71 off `surface`, `inkDim` 5.24:1.
		 * rowSelected #3B425C  panel hue, C* 16.91,
		 *                       the panel's cast + 4.0, floored at 5.0; +3.23 L*,
		 *                       ΔE00 3.57 off `surface` and 6.01 off
		 *                       `rowHover`; the pair ranks 1.04 `L*` and 9.1 `C*`,
		 *                       `inkDim` 5.04:1.
		 */
		rowHover: "#3E404C",
		rowSelected: "#3B425C",
