...
export const nord: ThemeDefinition = {
	...
	palette: {
		 ...
		 * 12.17 `C*` against the panel's 9.57 — the panel's own colour, one step lighter, which
		 * is what the operator asked for. ΔE00 from `surface` 2.60, from
		 * `elevated` 1.39, from `sunken` 7.57. Ink on this ground: `ink` 8.92:1,
		 ...
		 * chroma (10.86 `C*` against 9.57), 2 degrees off its hue, ΔE00 2.14 from
		 ...
		 * the step at 2.55 `L*`, and with the band's whole ceiling spent that reaches only
		 * ΔE00 2.14 — under the 2.5 floor every palette must hold, which is the
		 ...
		 * ceiling on the floor: this value adds 1.01 `C*` past the band's ceiling — 2.61 over
		 * the panel, at the panel's own hue, with the step unchanged at 2.58 `L*` — and that is
		 * what reaches ΔE00 2.60. It is the SMALLEST chroma that does, pinned by name in
		 * `HIGHLIGHT_OVER_BAND_PINS` and re-derived by `pnpm check-themes`, so the exception
		 * cannot drift into a licence to be loud.
		 *
		 * THE HOVER STEP IS THE COLLISION: 1.39 to `elevated` is under the field floor,
