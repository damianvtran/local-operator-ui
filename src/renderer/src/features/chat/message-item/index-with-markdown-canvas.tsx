 ...
 * Supports opening markdown files in the markdown canvas
 */
export const MessageItemWithMarkdownCanvas: FC<MessageItemProps> = memo(
	({ message, onMessageComplete, isLastMessage }) => {
		...
		const renderFileAttachments = (files: string[]) => {
			return files
				...
				{isSecurityCheck ? (
					<SecurityCheckHighlight isUser={isUser}>
						...
						>
							...
							{message.files && message.files.length > 0 && (
								<Box sx={{ mt: 2 }}>{renderFileAttachments(message.files)}</Box>
							)}
