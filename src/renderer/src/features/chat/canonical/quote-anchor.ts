 ...
 * control would cover.
 *
 * FLIPPING PUTS IT BELOW THE LAST LINE, NOT BELOW THE FIRST, so it can never
 * land ON highlighted text - the failure mode of a naive flip is a control
 * sitting over the second line of the sentence it is quoting.
 *
...
export type QuoteAnchorInput = {
	/** The highlight's own client rects, one per line, in document order. */
	lines: readonly Box[];
 ...
 * highlight's LAST line, so a highlight whose tail is under the fold puts the
 * flip's own position off screen, and the clamp is what keeps the press
 * reachable there.
 */
