...
type MessageHistoryState = {
   ...
   */
  setCurrentSequenceId: (conversationId: string, sequenceId: string | null) => void;
 ...
 */
export const useMessageHistoryStore = create<MessageHistoryState>((set, get) => ({
  ...
  getCurrentSequenceId: (conversationId) => {
    return get().historyByConversation[conversationId]?.currentSequenceId || null;
  },
...
export const useUserMessageHistory = (
   ...
   */
  const getPreviousMessage = (currentId: string | null): Message | undefined => {
