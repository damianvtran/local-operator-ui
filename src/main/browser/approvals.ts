...
export class ApprovalStore {
	...
	ensureTopLevelAccess(
		...
		if (authority) {
			// The once grant is CONSUMED here and nowhere else: re-consulting the grant
			// map later in the same command would let the 10-minute TTL lapse between
			// entry and navigation, turning a granted navigation back into a prompt the
			// agent thinks it already passed.
	 ...
	 *
	 * THE SINGLE SOURCE OF TRUTH, and the fix for a shipped defect (found by an
	 * independent end-to-end pass against v0.24.0): the gate and `request_access`
	 * used to answer from two different places — `ensureTopLevelAccess` from the
	 * grant, `requestAccess` from a resolved RECEIPT — and a receipt outlives the
	 * grant it was written for (15 minutes against a 10-minute once grant), so
	 ...
	 * agent was told it could proceed while the user was never asked.
	 *
	 ...
	 * consumes and this does not. Everything else is not authority: a spent grant, a
	 * revoked one, a receipt for either.
	 */
	...
	requestAccess(
		...
		const now = this.now();
		/*
		 * THE ANSWER COMES FROM LIVE AUTHORITY, and from nothing else. This replaces two
		 * reads: an `originAllowed` early return, and a receipt branch that answered
		 * `allowed` from a resolved decision. The receipt branch was the shipped defect
		 * — a receipt is a COPY of a decision with its own 15-minute TTL, so it answered
		 * `allowed` while the grant it stood for had been spent, revoked or forgotten,
		 * and the gate, reading the grant, refused. One predicate answers both questions
		 * now, which is why the next `open` cannot disagree with this answer.
		 */
		...
		// Same origin, same requester, already pending: idempotent, and the ORIGINAL TTL
		// is kept. Resetting it would let a polling agent extend the window
		// indefinitely. It sits after the live-authority read so an unspent grant still
	...
	accessStateFor(rawUrl: unknown, requester: string): Record<string, unknown> {
		...
		/*
		 * THE SYMMETRY WITH THE ALLOW PATH (the same shipped-build pass, its Q3): an
		 * allow is readable from live state once its entry is gone - the grant is still
		 * in force - and a deny has to be too. Without this the same flow read `allowed`
		 * for a yes and `none` for a no, and `none` is the word for "no request exists
		 * for you", which an agent narrates to the user as "the user has not answered"
		 * while the user has in fact refused. `superseded` is left alone: it is an
		 * explicit verdict about this requester's own request.
		 */
