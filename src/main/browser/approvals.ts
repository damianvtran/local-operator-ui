...
export class ApprovalStore {
	...
	originAllowed(url: URL): boolean {
		...
		// `hostGrants` is deliberately `undefined` here, matching `grantScopeFor`
		// below: the legacy shape it describes is the extension's 0.1.4-0.1.7 loopback
		// all-ports grant, this store is new, and the module turns an absent shape into
		// "no legacy grant" rather than into a second grant model with no data behind
		// it.
		//
		// FOUND WHILE BUILDING THE CHROME (this PR): the third argument used to be
		// `this.store.siteGrants`, which is the `hostGrants` slot. Every broad grant
		// was therefore invisible to this predicate, so an origin the user had approved
		// at DOMAIN scope was refused by the gate and re-prompted — the consent bar's
		// own "Allow all of <domain>" button would have been a lie the first time
		// somebody clicked it, and the failure direction was the safe one, which is why
		// the host PR's tests did not catch it: they assert the exact-origin and once
		// paths, and `grantScopeFor` (which passes the arguments correctly) is the call
		// the approvals list reads. `scripts/browser-chrome.test.mjs` pins the domain
		// path end to end.
