...
export class ConsoleHost {
	...
	create(options: CreateSurfaceOptions): CreateSurfaceResult {
		...
		if (entry.record.retain) this.persist(surface);
		this.options.log(
	...
	async input(
		...
		if (raw.length > MAX_INPUT_BYTES) {
			...
			throw new ConsoleError(
				...
				{ accepted: 0, limit: MAX_INPUT_BYTES, bytes: raw.length },
			);
		}
		const bracketed =
	...
	async keys(
		...
		for (const bytes of encoded) {
			if (bytes) this.writeToPty(entry, runtime, bytes);
		}
		return { accepted: true, encoded: names };
	}
	...
	resize(surface: string, cols: number, rows: number): Record<string, unknown> {
		...
		if (entry.record.sizing === "fixed") {
			throw new ConsoleError(
				...
				{ reason: "fixed", cols: entry.record.cols, rows: entry.record.rows },
			);
		}
	...
	setSecure(surface: string, on: boolean): Record<string, unknown> {
		...
		if (on && entry.runtime) {
			// The bytes received while the window is open are not logged: the log is
			// what survives to disk and to a replay, and this switch exists because a
			// person wants certainty that what is on screen does not. The residual is
			// stated rather than hidden: the record (the grid) still holds what a
			// person can see, and a cooperating agent that reads after the window
			// closes would see it — which is why the refusal below is documented as
			// advisory (§11.4.4).
			entry.runtime.log.clear();
		}
		this.persist(surface);
		this.options.onChanged?.();
		return { secure: on };
	}
	...
	async close(
		...
		if (runtime && entry.record.running) {
			...
			exitCode = (await this.terminate(runtime)) ?? exitCode;
		}
	 ...
	 * than waited on.
	 */
	private async terminate(runtime: SurfaceRuntime): Promise<number | null> {
		const exit = new Promise<number>((resolve) => {
			runtime.exitWaiters.push(resolve);
		});
		runtime.pty?.kill();
		...
		if (code === null) runtime.pty?.kill("SIGKILL");
		return code;
	}
	...
	private writeToPty(
		...
		runtime.pty?.write(Buffer.from(bytes).toString("binary"));
	...
	private requireRunning(entry: ConsoleRegistryEntry<SurfaceRuntime>): void {
		if (!entry.record.running) {
			throw new ConsoleError(
				...
				{ exit_code: entry.record.exitCode, live: entry.record.live },
			);
		}
	}
	...
	/** Persist one surface's bytes and sidecar, and remember that it was seen. */
	private persist(surface: string): void {
		const entry = this.registry.require(surface);
		const runtime = entry.runtime;
		...
		const meta: ConsoleHistoryMeta = {
			...
			// rewritten rather than appended, so it carries what is true now.
		...
		for (const entry of this.registry.clear()) {
			...
			if (entry.record.retain) {
				const meta: ConsoleHistoryMeta = {
					...
					argv: entry.record.argvTail ? entry.record.argvTail.split(" ") : [],
					cwd: entry.record.cwd,
					cols: entry.record.cols,
					rows: entry.record.rows,
					created_at: this.createdAt.get(entry.record.surface) ?? this.now(),
					last_seen_at: entry.record.lastActivity,
					exit_code: entry.record.exitCode,
					truncated: runtime.log.truncated,
				};
				this.options.history?.writeMeta(meta);
			}
